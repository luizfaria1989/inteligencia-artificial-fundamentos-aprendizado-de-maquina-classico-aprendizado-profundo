...
\pgfmathdeclarefunction{gamma_pdf}{3}{%
  \pgfmathparse{(#1^(#2-1) * exp(-#1/#3)) / (#3^#2 * (#2-1)!)}%
}
...
% No seu preâmbulo (substitua o \usepackage{acronym})
\usepackage[
    toc,      % Põe no sumário
]{glossaries-extra}

% Comando obrigatório para inicializar o pacote
\makenoidxglossaries
...
% --- Definição da Cor ---
% Vamos definir uma cor "tech". Um azul ou cinza escuro funciona bem.
% (Valores RGB de 0 a 255)
\definecolor{ChapterTechBlue}{RGB}{0, 82, 155} 
...
% --- Formatação da Seção (Corrigido) ---
% O \Large foi movido para o primeiro argumento { }
\titleformat{\section}
  {\normalfont\sffamily\bfseries\color{black}\Large}
  {\thesection} 
  {0.7em} 
  {}

% --- Formatação da Subseção (Corrigido) ---
% O \large foi movido para o primeiro argumento { }
...
% --- Espaçamento ---
% Ajusta o espaço ANTES e DEPOIS dos títulos
\titlespacing*{\section}{0pt}{3.5ex plus 1ex minus .2ex}{2.3ex plus .2ex}
\titlespacing*{\subsection}{0pt}{3.25ex plus 1ex minus .2ex}{1.5ex plus .2ex}

\renewcommand*{\glossaryname}{Glossário}

% Pacotes Essenciais para Matemática e Tabelas
\usepackage{amsthm} % Para teoremas e provas
\usepackage{booktabs} % Para tabelas profissionais
\usepackage{xcolor} % Opcional, para realce

% Configuração dos Ambientes (Teoremas, Proposições, Definições)
% O estilo 'definition' deixa o texto normal (não itálico), o que é melhor para leitura longa.
\theoremstyle{definition}
\newtheorem{definition}{Definição}[chapter] % Reseta numeração por capítulo
...
% Personalização do símbolo QED (quadradinho preto ao final da prova)
