...
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}[font=\small] % Define uma fonte menor para a figura
        \begin{axis}[
            title={Comparativo de Trajetórias de Otimização},
            xlabel={Parâmetro $\theta_1$},
            ylabel={Parâmetro $\theta_2$},
            view={0}{90}, % Visão de cima (2D)
            enlargelimits=false,
            axis lines=none, % Esconde os eixos para um visual mais limpo
            width=12cm,
            height=10cm,
            clip=false % Permite que os labels fiquem fora da área do gráfico
        ]

        % 1. Plot das curvas de nível (função de custo)
        \addplot3[
            contour gnuplot={
                levels={0.5, 2, 5, 10, 18, 30},
                labels=false
            },
            thick,
            - % Linhas sólidas
        ] {1.5*x^2 + y^2 + 0.5*x*y}; % Uma função de custo elíptica
        ...
        \draw[-{Stealth[length=2mm]}, blue, thick, line cap=round,
            decorate, decoration={
                random steps,
                segment length=6pt, % Comprimento de cada "passo" aleatório
                amplitude=3pt      % Amplitude do "zigue-zague"
            }
...
\begin{equacaodestaque}{Gradiente Acelerado de Nesterov (NAG)}
    \begin{aligned}
        \theta_{\text{lookahead}} &= \theta_t + \beta(\theta_t - \theta_{t-1})   && \text{1. Estima-se uma posição futura (\textit{lookahead}),} \\
                &                               && \text{   avançando na direção do momento anterior.} \\[1em]
        \theta_{t+1} &= \theta_{\text{lookahead}} - \eta \nabla_{\theta} J(\theta_{\text{lookahead}}) && \text{2. O gradiente é calculado nessa posição futura} \\
                &                               && \text{   e a atualização dos parâmetros é feita a partir dela.}
    \end{aligned}
...
\begin{tikzpicture}[
    ...
    \node[font=\scriptsize] at (0,0) {Mínimo Global};

    % --- Coordenadas dos Pontos ---
    % Posição inicial na iteração t
    \coordinate (theta_t) at (-3.5, 2.0);
    % Ponto de "lookahead" (estimativa futura baseada no momento)
    \coordinate (lookahead) at (-1.0, 0.9);
    % Posição final na iteração t+1
