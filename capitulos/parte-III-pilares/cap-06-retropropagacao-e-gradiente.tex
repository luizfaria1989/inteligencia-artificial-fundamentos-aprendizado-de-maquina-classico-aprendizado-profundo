...
\begin{itemize}
    \item $\theta_t$: representa as coordenadas do ponto atual.
          \nomenclature[A@thetat]{$\theta_t$}{Vetor de parâmetros na iteração $t$ (ponto atual)}

    \item $\theta_{t+1}$: representa as coordenadas do próximo ponto.
          \nomenclature[A@thetat+1]{$\theta_{t+1}$}{Vetor de parâmetros na iteração $t+1$ (próximo ponto)}

    \item $\eta$: representa o tamanho do passo, também chamado de taxa de aprendizado.
          \nomenclature[A@eta]{$\eta$}{Taxa de aprendizado (ou tamanho do passo)}

    \item $\nabla f(\theta_t)$: representa o vetor gradiente da função $f$ calculado no ponto atual.
          \nomenclature[A@nabla]{$\nabla f(\theta_t)$}{Vetor gradiente da função $f$ em relação a $\theta_t$}
\end{itemize}
...
A implementação do gradiente estocástico consiste em utilizar como base a classe que representa o método do gradiente padrão, contudo, ao invés de receber como entrada todos os parâmetros do modelo para fazer as atualizações, essa função irá receber somente um parâmetro por vez, acelerando as atualizações mas também gerando caminhos mais aleatórios. Dessa forma, não será visto um bloco de código para esse método de otimização.
...
Semelhante ao método do gradiente estocástico que recebe um parâmetro por vez para ser atualizado e por isso, sua implementação consiste em utilizar o método do gradiente padrão porém passando um parâmetro, o gradiente em mini-batch segue a mesma ideia. Porém neste caso ele receberia como entradas um \textit{batch} de parâmetros por vez.
...
Antes de implementar o NAG usando python, é possível desenvolver o seu algortimo utilizando pseudocódigo e suas fórmulas da equação \ref{eq:gradiente-acelerado-de-nesterov}. Assim, tem-se o bloco \ref{alg:gradiente-acelerado-de-nesterov}.
...
Tendo o algoritmo do gradiente acelerado de Nesterov desenvolvido, é possível utilizá-lo como base para construir a sua classe em python.
...
Para testar esse novo algoritmo, os pesquisadores \textcite{AdamMethod} propraram uma série de experimentos, sendo alguns deles envolvendo uma regressão logística, uma rede neural multi-camadas e uma rede convolucional.
...
\begin{codelisting}{Classe completa do otimizador Adam}{gd_class}
import numpy as np
from optimizers.base import Optimizer
