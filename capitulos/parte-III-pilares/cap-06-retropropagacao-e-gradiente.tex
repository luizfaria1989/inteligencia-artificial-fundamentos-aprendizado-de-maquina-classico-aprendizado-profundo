...
Conforme o tempo foi passando, o SGD provou ser uma excelente alternativa para encontrar pontos de mínimo em uma função de perda quando comparado com o método do gradiente tradicional. Dessa forma, ele continua sendo um dos otimizadores que ainda é empregado até hoje na construção de redes neurais. A sua fórmula pode ser vista na expressão \ref{eq:metodo-do-gradiente-estocastico}.
...
\begin{algorithm}[H]
    ...
    \label{alg:nesterov-accelerated-gradient}
    \begin{algorithmic}[1]

        \State $\textbf{g_t} \leftarrow f(\theta_{t-1} - \eta \mu m_{t-1})$
        \State $m_t \leftarrow \mu m_{t-1} + g_t$
        \State $\theta_t \leftarrow \theta_{t-1} - \eta m_t$
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $\textbf{g_t} \leftarrow \nabla_{\theta_{t - 1}} f(\theta_{t-1})$
        \State $\textbf{n_t} \leftarrow \textbf{n_{t-1}} + \textbf{g_t}^2$
        \State $\theta_t \leftarrow \theta_{t-1} - \eta \frac{g_t}{\sqrt{n_t} + \epsilon}$
