...
\begin{algorithm}[H] % A opção [H] tenta colocar o algoritmo exatamente aqui
\caption{Estimação de Momento Adaptativa com Aceleração de Nesterov (Nadam)}
\label{alg:nadam}
\begin{algorithmic}[1] % O [1] habilita a numeração das linhas
...
\begin{algorithm}[H]
    \caption{Otimizador Lion (nosso)}
    \label{alg:lion}
    \begin{algorithmic}[1]

    \State \textbf{Dado:} $\beta_1, \beta_2, \lambda, \eta, f$
    \State \textbf{Inicialize:} $\theta_0, m_0 \leftarrow 0$

    \While{$\theta_t$ não convergir}
        \State $g_t \leftarrow \nabla_\theta f(\theta_{t-1})$ \Comment{Obter gradientes da função de perda $f$}
        \State
        \State \textit{// Atualizar parâmetros do modelo}
        \State $c_t \leftarrow \beta_1 m_{t-1} + (1 - \beta_1) g_t$ \Comment{Interpolar momento e gradiente para a direção da atualização}
        \State $\theta_t \leftarrow \theta_{t-1} - \eta \cdot (\text{sign}(c_t) + \lambda \theta_{t-1})$ \Comment{Atualizar pesos usando o sinal e o decaimento de peso}
        \State
        \State \textit{// Atualizar EMA (Média Móvel Exponencial) de $g_t$}
        \State $m_t \leftarrow \beta_2 m_{t-1} + (1 - \beta_2) g_t$ \Comment{Atualizar o estado do momento para o próximo passo}
    \EndWhile
