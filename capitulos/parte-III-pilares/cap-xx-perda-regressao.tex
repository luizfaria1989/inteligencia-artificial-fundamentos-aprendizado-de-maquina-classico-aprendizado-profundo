...
Para garantir o funcionamento do algoritmo da retropropagação é necessário utilizar uma função de perda em conjunto. As funções de perda são responsáveis por definir a superfície que o modelo deverá percorrer ao ser otimizado na busca por pontos de mínimo. Elas se dividem em finalidades, existindo funções de perda específicas para lidar com tarefas de regressão (quando querem prever qual será um determinado valor com base um conjunto de dados), para tarefas de classificação (binária ou multi-classe), e para outros tipos de tarefas. 

Este capítulo tem foco em explicar as diferentes funções de perda que podem ser utilizadas em problemas de regressão. Ele apresenta quatro seções principais. A primeira explica funções de perda padrão, que são utilizadas quando os dados seguem a distribuição Normal. Em seguida, é dedicada uma seção para explicar perdas assimétricas, para casos em que queremos penalizar em maior grau as subestimações ou as superestimações.

A terceira seção explica a perda $\epsilon$-insensível, a função de perda para máquinas de vetores de suporte de classificação. A última seção aborda três funções de perda que são utilizadas nos casos que não seguem a distribuição Normal, são elas: a perda de Poisson, perda Gamma e perda de Tweedie. Por fim, é apresentada uma tabela, destacando as principais características das perdas apresentadas neste capítulo.
...
\begin{equation}
    \nabla_{\hat{y}_j} \Loss_{\text{mse}} (y_j, \hat{y}_j) = \left( \frac{\partial \Loss_{\text{MSE}}}{\partial y_j}, \frac{\partial \Loss_{\text{MSE}}}{\partial \hat{y}_j} \right)%
...
\begin{equacaodestaque}{Gradiente do erro absoluto médio em Relação à predição}
    \nabla_{\hat{y}_j}\Loss_{\text{MAE}}(y_j, \hat{y}_j) = 
    \begin{cases} 
...
\begin{equacaodestaque}{Gradiente da perda de Huber em relação à predição}
    \nabla_{\hat{y}_j}\Loss_{\delta} (y_j, \hat{y}_j) = 
    \begin{cases} 
...
\begin{equacaodestaque}{Gradiente da perda log-cosh em relação à predição}
    \nabla_{\hat{y}_j}\Loss_{\text{log-cosh}} = \tanh(\hat{y}_j - y_j)%
...
\begin{equacaodestaque}{Gradiente do erro quadrático médio logarítmico em relação à predição}
    \nabla_{\hat{y}_j}\Loss_{\text{MSLE}} (y_j, \hat{y}_j) = -\frac{2}{N} \cdot \frac{\ln(y_j + 1) -\ln(\hat{y}_j + 1)}{\hat{y}_j + 1}%
...
\begin{equacaodestaque}{Gradiente da perda de Poisson em relação à predição}
    \nabla_{\hat{y}_j}\Loss_{\text{Poisson}} = 1 -\frac{y_j}{\hat{y}_j} = \frac{\hat{y}_j -y_j}{\hat{y}_j}%
...
\begin{equacaodestaque}{Gradiente da perda de Poisson em relação à predição com exp como função de conexão}
    \nabla_{\hat{y}_j}\Loss_{\text{Poisson}} = \hat{y}_j - y_j%
...
Para encontramos a perda Gamma, devemos derivá-la da FDP da sua respectiva distribuição. O primeiro passo é considerar uma parametrização diferente para escrever a FDP, queremos ter a função em termos da média esperada $\hat{y}_j$. Sabemos que para a parametrização da Equação~\ref{eq:gamma-distribution-pdf} a média é calculada por
...
\begin{figure}[h!]
    ...
    \caption{Visualização da perda Gamma para diferentes valores reais $y_j$.}%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \begin{equation}
        L(z_j) = \frac{y_j}{e^{z_j}} + z_j
    \end{equation}

    A grande vantagem dessa transformação é a convexidade. A segunda derivada dessa função é $L''(z) = y_j e^{-z_j}$, que é estritamente positiva para todo $y_j > 0$, garantindo um único mínimo global.
\end{description}
...
\begin{equacaodestaque}{Gradiente da perda Gamma em relação à predição}
    \nabla_{\hat{y}_j}\Loss_{\text{Gamma}} = \frac{1}{\hat{y}_j} -\frac{y_j}{\hat{y}_j^2} = \frac{\hat{y}_j -y_j}{\hat{y}_j^2}%
    \label{eq:gamma-loss-derivada}
\end{equacaodestaque}

Entretanto, também podemos considerar a derivação para o cenário em que a perda Gamma é aplicada em conjunto com a função logaritmo, neste caso, temos a Equação~\ref{eq:gradiente-gamma-link}. A derivada, é simplesmente a diferença relativa entre 1 e a razão entre o valor real $y_j$ e o previsto $\hat{y}_j$


\begin{equacaodestaque}{Gradiente da perda Gamma em relação à predição com $\log$ como função de ligação}
    \nabla_{z_j} \Loss = 1 - \frac{y_j}{\hat{y}_j}%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \begin{equation}
        L(z_j) = -y_j \frac{e^{z_j(1-p)}}{1-p} + \frac{e^{z_j(2-p)}}{2-p}
    \end{equation}
...
\begin{equacaodestaque}{Gradiente da perda de Tweedie em relação à predição}
    \nabla_{\hat{y}_j} \Loss_{\text{Tweedie}} = \hat{y}_j^{-p}(\hat{y}_j -y_j)%
...
\begin{equacaodestaque}{Gradiente da perda de Tweedie em relação ao logit}
    \nabla_{z_j} \Loss_{\text{Tweedie}} = \hat{y}_j^{1-p} (\hat{y}_j - y_j)%
    \label{eq:tweedie-loss-derivada-logit}
\end{equacaodestaque}

Como a Figura~\ref{fig:tweedie-loss-derivada} aponta a derivada da perda de Tweedie compartilha características em comum com as perdas de Poisson e Gamma. Uma delas é o fato do gradiente tender a zero, quando o valor predito aumenta (também explicitado ao discutir as características da perda). Além disso, conforme os valores reais se aproximam de zero, a curva também se desloca para o eixo das ordenas. $y_j$ indica não só o ponto de mínimo da perda, mas também para qual direção a assimetria da função irá se deslocar.
