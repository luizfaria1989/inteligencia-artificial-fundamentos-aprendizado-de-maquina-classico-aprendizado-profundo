...
A distribuição de Poisson é dada pela Equação~\ref{eq:poisson-distribution-pmf}, onde o espaço amostral é conjunto de inteiros não-negativos. \textcite{GeneralizedLinearModels} aponta que essa distribuição não possui uma limite superior nos valores que podem ser observados, diferente da distribuição Normal.
...
A log-verossimilhança da perda da distribuição de Poisson está na Equação~\ref{eq:log-verossimilhanca-negativa-de-poisson}, é a partir dela que chegaremos na definição da perda de Poisson.

\begin{equacaodestaque}{Log-verossimilhança negativa para a distribuição de Poisson}
    l_j (\lambda_j) =  y_j \log(\lambda_j) - \lambda_j \approx y_j \log(y_j) - y_j - 9y^{1/3}_j (\lambda_j^{1/3} - y_j^{1/3})^2/2 %
...
Da função log verossimilhança da distribuição de Poisson, Equação~\ref{eq:log-verossimilhanca-negativa-de-poisson}, tomamos a sua aproximação, ou seja

\[
    y_j \log(\lambda_j) - \lambda_j
\]

Dela, multiplicamos toda a expressão por $-1$. Assim, encontramos a Equação~\ref{eq:poisson-loss}, referente ao cálculo da perda de poisson. Neste caso, o termo $\lambda_j$ é dado pelo valor predito pelo modelo $\hat{y}_j$.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda de Poisson pertence à classe $C^\infty$ (infinitamente diferenciável) em seu domínio de definição $(0, \infty)$ (ver Apêndice~\ref{ap:deducoes-poisson-loss}).
    \item[Convexidade:] A perda de Poisson é convexa, e estritamente convexa se $y > 0$ (ver Apêndice~\ref{ap:deducoes-poisson-loss}).  
    ...
    \item[Não-negatividade e link functions:] \textcite{LossesArticle} explicam que como dados de contagem não podem ser negativos, deve-se atentar com os valores preditos pelo modelo $\hat{y}_j$, pois eles deverão estar restritos em $\hat{y}_j > 0$. Em modelos lineares generalizados a função exponencial é comumente utilizada como uma link function para garantir que esses valores se encaixem nesse intervalo \parencite{LossesArticle}. Dito isso, é possível discutir uma versão alternativa para descrever a perda de Poisson considerando que $\text{exp}$ está sendo usada como uma link function.
