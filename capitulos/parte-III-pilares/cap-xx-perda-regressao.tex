...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \midrule
            Erro quadrático médio (MSE) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, estritamente convexa,  \\
            \addlinespace
            Erro absoluto médio (MAE) & Contínua ($C^0$), robusta a \textit{outliers}, convexa \\
            \addlinespace
            Perda de Huber (\textit{Huber loss}) & Continuamente diferenciável ($C^1$), robusta a \textit{outliers}, convexa, \\
            \addlinespace
            Perda log-cosh (\textit{Log-cosh loss}) & Infinitamente diferenciável ($C^{\infty}$), robusta a \textit{outliers}, estritamente convexa, \\
            \addlinespace
            Erro quadrático logarítmico médio (MSLE) & Infinitamente diferenciável ($C^{\infty}$), robusta a \textit{outliers}, estritamente convexa no espaço logaritmo \\
            \addlinespace
            Perda quantílica (\textit{Quantile loss}) & Contínua ($C^0$), robusta a \textit{outliers}, convexa, \\
            \addlinespace
            Perda epsilon-insensível (\textit{$\epsilon$-insentive}) & - \\
            \addlinespace
            Perda de Poisson (\textit{Poisson deviance}) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, convexa (e estritamente convexa se $y > 0$), \\
            \addlinespace
            Perda Gamma (\textit{Gamma deviance}) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, não convexa (solução: usar $\log$ como função de ligação), \\
            \addlinespace
            Perda de Tweedie (\textit{Tweedie deviance}) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, não convexa (solução: usar $\log$ como função de ligação),  \\
            \addlinespace
        \end{tabularx}
