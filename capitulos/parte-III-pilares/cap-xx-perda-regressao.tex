...
Para isso, esse capítulo busca explicar algumas dos diferentes tipos de funções de perda, mais precisamente as funções de perda para tarefas de regressão. Portanto, o capítulo pode ser dividido em quatro grandes partes: funções de perda para propósitos gerais, funções de perda para medir o erro relativo, funções que vão além do cálculo da média dos erros, por último funções que são utilizadas para problemas que seguem outros tipos de distribuição (como as distribuições de Poisson e de Tweedie).
...
Voltando para analogia do jogo dos dardos, é possível estendê-la para explicar o erro quadrático médio. Imagine que no jogo de dardos todos os jogadores começam com 1.000 pontos, e que vão perdendo conforme vão errando os lançamentos. Além disso, você definiu uma regra que diz que o erro (ou débito dos pontos totais) será dado pelo quadrado da distância entre o centro até o dardo jogado. Diante disso, jogadores que são muito precisos, e com isso acertam dardos mais próximos do centro perdem poucos pontos, contudo, aqueles jogadores que são mais desleixados e acertam longe do centro acabam perdendo muitos pontos, porque seu além de já perderem muitos pontos por estarem longe do centro, essa distância ainda será elevada ao quadrado.

Essa ideia de elevar o erro ao quadrado é o grande motivador para entender o cálculo do \textit{MSE}. Dito isso, agora cabe finalmente analisar essa função de fato, em primeiro lugar será vista a sua fórmula, a qual está apresentada na Equação~\ref{eq:mse}.
...
O gráfico do erro quadrático médio está representado na Figura~\ref{fig:mse}. Existem duas subfiguras, a primeira à esquerda, Figura~\ref{fig:mse-2d} mostra uma visão em duas dimensões dessa função de perda. Contudo, pelo fato do cálculo do erro ser uma função de duas variáveis, que apresenta como entradas o valor real para a saída ($y_j$) e o valor previsto pelo modelo ($\hat{y}_j$), a sua representação real deve ser feita com o uso de três dimensões, como é mostrado na Figura~\ref{fig:mse-3d}.
...
Cabe agora discutir algumas das principais característica dessa função de perda, destacando as suas vantagens e como ela pode ser útil ao ser utilizada na construção de um modelo de regressão.
...
Os gráficos da derivada do erro quadrático médio (mostrada na Equação~\ref{eq:mse-derivada}) estão presentes na Figura~\ref{eq:mse-derivada}. A sua derivada também é apresentada em vistas em duas e três dimensões, isso será algo recorrente nas representações das funções desse capítulo.
...
Cabe também discutir a raiz do erro quadrático médio, vista na Equação~\ref{eq:rmse-metric}. Ela é uma função derivada do \textit{MSE}, cuja a sua diferença é que é feito o cálculo de uma raiz com o resultado do erro quadrático médio.
...
Dito isso, finalmente será estudado algumas aplicações dessas funções em algoritmos de aprendizado de máquina.
...
Além de estar presente nas deduções do algoritmo da retropropagação como uma função de perda, o erro quadrático médio acaba sendo uma função bem versátil para ser aplicada em problemas de regressão. Não somente isso, mas seu uso não se restringe a somente uma função de perda que será utilizada como guia para a otimização do modelo. O \textit{MSE} também é uma ótima métrica para avaliar o desempenho de um modelo de regressão. E, junto do \textit{MSE} está o \textit{RMSE}, juntas, essas duas funções se tornam ótimas escolhas para medir como um modelo está performando.
...
Conhecendo a função de perda \textit{MSE}, cabe discutir outras funções de perda que também são igualmente importantes. Para isso, a próxima seção apresenta o erro absoluto médio, que é uma alternativa para o erro quadrático médio, tendo como principal diferença o jeito que lida com \textit{outliers}.
...
Já os gráficos do erro absoluto médio estão representados na Figura~\ref{fig:mae}. Seguindo a mesma lógica: na subfigura à esquerda, está a visão em duas dimensões, e na subfigura à direita, a superfície no espaço.
...
\begin{equacaodestaque}{Derivada Parcial do Erro Absoluto Médio (\textit{MAE}) em Relação à $\hat{y}_j$}
...
A \textit{Huber Loss}, representada com as notações deste livro, está retratada na Equação~\ref{eq:huber-loss}.
...
Após a leitura dessa seção, foi possível ver que a perda de Huber é uma excelente alternativa para os casos em que deseja-se controlar os pontos em que o cálculo da perda deve atuar de forma severa (usando termos quadráticos, semelhante a perda L2). E a partir de quais casos esse cálculo pode ser menos punitivo (usando termos lineares, semelhante a perda L1). Contudo, foi visto que ela tem um problema, os pontos em que essas funções se juntam gera uma descontinuidade, atrapalhando a derivação dessa perda de Huber. Para resolver esse problema da descontinuidade, pode ser utilizada como alternativa a perda Log-Cosh, tópico principal da próxima seção.
...
\begin{equacaodestaque}{Derivada Parcial da Perda Log-Cosh em Relação à Predição $\hat{y}_j$}
...
Além das aplicações discutidas no início da seção sobre a perda Log-cosh, ela também está presente em várias outras pesquisas. Para isso, essa seção explica algumas aplicações dessa função de perda para resolver problemas no contexto de aprendizado de máquina.
...
\begin{equacaodestaque}{Derivada Parcial do Erro Quadrático Médio Logarítmico (\textit{MSLE}) em Relação à $\hat{y}_j$}
...
Também há uma outra função de perda que também não trabalha com o cálculo da média dos erros, ela é a $\epsilon$-insensível, sendo uma função essencial para a tarefas de regressão para máquinas de vetores de suporte. Neste caso, ao utilizar essa função, é possível definir uma ``margem de tolerância'' em que caso o erro esteja dentro dessa média, a perda não irá penalizar o modelo.

Visto isso, a seção seguinte explica a perda quantílica.

\subsection{Perda Quantílica (Quantile Loss)}%
\index{Funções de Perda!Perda Quantílica (\textit{Quantile Loss})}

Antes de entender a perda quantílica de fato, cabe ver o conceito em que ela surgiu. No artigo \textit{Regression Quantiles} dos autores \textcite{regression-quantiles} estava sendo analisada uma nova técnica de regressão que se baseava nos quantis de uma distribuição. Como motivação para esse estudo, os autores discutem a ideia de que ``todos acreditam a lei de Gauss para os erros, porque eles acreditam que é um teorema matemático, e os matemáticos acreditam porque é um fato experimental'' \parencite{regression-quantiles}. Contudo, isso não é uma verdade absoluta, existem casos, como os vistos na introdução dessa seção que fogem da distribuição normal. Como consequência, técnicas mais conhecidas, como o \textit{MSE} para o cálculo dos erros com base na média, não são ideais para minimizar o erro de um modelo.
...
\begin{equacaodestaque}{Derivada Parcial da Perda Quantílica em Relação à Predição $\hat{y}_j$}
...
Já o gráfico da derivada da perda quantílica está na Figura~\ref{fig:quantile-loss-derivada}, neste caso está sendo apresentado somente a sua visualização em duas dimensões. Perceba que o gráfico em que $\tau = 0.5$ é igual o gráfico da derivada do erro absoluto médio.
