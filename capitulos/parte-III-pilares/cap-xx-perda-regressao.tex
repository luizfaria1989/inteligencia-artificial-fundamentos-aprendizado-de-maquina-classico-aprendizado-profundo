...
\begin{itemize}
    \item \textbf{Robustez a \textit{outliers}:} A perda de Huber não penaliza erros grandes de forma quadrática quando comparada com o erro quadrático médio. Portanto, os \textit{outliers} não conseguem enviesar drasticamente o cálculo da perda dependendo do valor de $\delta$ escolhido \parencite{LossesArticle};
    \item \textbf{Convexa:} A perda de Huber é uma função convexa. Isso é retratado na Figura~\ref{fig:huber-loss}, os seus gráficos seguem o formato de um funil, comum em funções convexas. Essa característica é uma vantagem para a perda de Huber, pois facilita com que os otimizadores encontrem os pontos de mínimo. Contudo, em redes neurais, devido às transformações não-lineares que ocorrem no modelo, a função de perda pode deixar de ser convexa;
