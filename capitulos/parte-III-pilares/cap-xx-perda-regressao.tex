...
como visto no Capítulo~\ref{cap:retropropagacao-gradiente}. As definições que calculam a média das perdas para todo o \textit{batch} também são chamadas de funções de custo, como explica xxx. Nesta obra, trataremos esses termos intercambiavelmente.
...
Ao utilizar o subgradiente para calcular a derivada parcial do erro absoluto médio em relação à predição $\hat{y}$ encontramos a Equação~\ref{eq:mae-derivada}.
...
\end{equacaodestaque}
 ...
 Pelo fato do MAE ser uma função que não é diferenciável em todos o seu domínio, isso reflete no jeito com que o gráficos da derivada são representados, os quais estão na Figura~\ref{fig:mae-derivada}. Existem, na derivada pontos de descontinuidade, fazendo com que o gráfico dessa função se assemelhe ao gráfico da função de ativação degrau unitário. Esses pontos de descontinuidade, estão exatamente em zero, justamente no ``bico'' do MAE.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda de Huber é uma função de classe $C^1$ (ver Apêndice~\ref{ap:deducoes-huber-loss}), \textbf{suave}. Isso implica em facilidades para encontrar as derivadas parciais e o vetor gradiente dessa função de perda.
...
\[
    \frac{d}{de} \log(\cosh(e)) = \frac{1}{\cosh(e)} \cdot \sinh (e) \quad \text{com} e = \hat{y} - y
\]
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Aplicação 3 (Área):] \textcite{AnEffectiveMethodForDetectingUnknowTypes} propõem um modelo de \textit{deep learning} chamado de \textit{LVAE (Log-Cosh Variational Autoencoder)} para detectar tipos desconhecidos de ataques em redes. A perda log-cosh é utilizada como a função de perda de reconstrução dentro do modelo \textit{variational autoencoder} \parencite{AnEffectiveMethodForDetectingUnknowTypes}.
\end{description}
...
O \textbf{erro quadrático médio logarítmico}, também chamado de \textbf{\textit{mean squared logarithmic error}} (\textbf{MSLE}), é calculado pela Equação~\ref{eq:msle-loss}. Ele avalia o erro como sendo a diferença dos logaritmos naturais do valor real $y$ com a predição $\hat{y}$.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Foco no erro relativo (percentual):] Quando o MSLE é utilizado como função de perda, diferente das outras vistas até agora que medem o erro absoluto, o MSLE mede o erro percentual/relativo. Isso acontece por conta dos uso dos logaritmos dessa função, que são responsáveis por comprimir a escala dos dados. Portanto, erros em que o valor predito é 50\% menor que o real são mais penalizados em que cenários nos quais o valor predito é 5\% menor.
...
Na equação~\ref{eq:msle-derivada} está a derivada do erro quadrático médio logarítmico em relação os valores preditos pelo modelo $\hat{y}$.

\begin{equacaodestaque}{Gradiente do erro quadrático médio logarítmico em relação à predição}
...
\begin{figure}[h!]
    ...
    \caption{Exemplos de distribuições assimétricas (desenhadas) onde a média (vermelho) e a mediana (verde) divergem.}%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Aplicação 3 (Área):]  \textcite{WINKELMANN2006131} usa regressão quantílica para contagens para estimar o efeito de uma reforma na saúde sobre a frequência de consultas médicas individuais. O autor justifica a escolha da regressão quantílica, e não uma técnica mais comum, como a dos mínimos quadrados, pois ele considera que esse é um problema em que os efeitos da reforma são diferentes em cada uma das partes da distribuição \parencite{WINKELMANN2006131}.
\end{description}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}]  
    \item[Um ensaio biológico sobre tuberculina:] rA distribuição de poisson com a sua função de log-verossimilhança é utilizada para descrever os casos de tuberculina em população de bovinos;
    \item[Um estudo sobre o dano das ondas em navios cargueiros:] xxx utilizam a distribuição de poisson assim como o cálculo da log-verossimilhança para estimar o dano das ondas em navios cargueiros.
\end{description}
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \addlinespace
            Perda epsilon-insensível (\textit{$\epsilon$-insentive}) & Contínua ($C^0$), robusta a \textit{outliers}, convexa, promove gradientes esparsos, pode ter o tamanho do tubo $\epsilon$ ao ajustar o hiperparâmetro $\epsilon$, \\
            \addlinespace
