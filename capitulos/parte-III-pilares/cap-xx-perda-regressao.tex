...
A perda log-cosh atua de forma parecida com a perda de Huber, ilustrado no comportamento da função nos gráficos da Figura~\ref{fig:log-cosh-loss}. Para valores em que a diferença da saída do modelo e o rótulo real ($y_j - \hat{y}_j$) é pequena, ela tem um comportamento que se assemelha à uma função quadrática (como o \textit{MSE}). Além disso, conforme o resultado dessa diferença de valores aumenta, a perda log-cosh assume um comportamento parecido com o de uma função linear (como o \textit{MAE}). Em testes realizados por \textcite{StatisticalPropetiesLogCosh}, a perda log-cosh foi comparada com a perda de Huber, e foi verificado que as estimativas dessas funções bem como os erros padrões apresentam resultados similares. Consequentemente, ela pode ser uma alternativa a ser considerada caso sejam encontrados problemas ao utilizar a perda de Huber, mas ainda é desejável manter a variação no cálculo do erro do modelo.
...
Ao derivar a perda log-cosh, encontra-se o clássico gráfico de uma função sigmoidal, vista na Figura~\ref{eq:log-cosh-derivada}. O gráfico mostra como será o comportamento do gradiente conforme o erro aumenta, neste caso, ele é cíclico, estando em um intervalo $[-1, 1]$. Ao retornar valores positivos e negativos no gradiente, é possível garantir uma convergência mais rápida para os pontos de mínimo da função de perda (esse fato está evidenciado no Capítulo~\ref{cap:ativacao-sigmoidais}).
...
Todas as funções vistas até o momento neste capítulo calculam o erro de forma absoluta. Elas não lidam com o erro de forma relativa. Como consequência, um erro de R\$ $50,00$ em uma conta de padaria tem o mesmo grau que um erro da mesma magnitude para validar o aluguel de uma casa. Vão existir cenários que será mais interessante, considerar o erro de forma relativa, punindo em maior peso as subestimações feitas pelo modelo. Para isso, uma solução é utilizar o erro quadrático logarítmico médio.
