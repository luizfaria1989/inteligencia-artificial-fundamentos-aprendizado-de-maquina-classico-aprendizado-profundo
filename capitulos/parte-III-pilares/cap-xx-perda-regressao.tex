...
\begin{equacaodestaque}{Derivada parcial da perda Gamma em relação à predição}
...
\begin{equacaodestaque}{Derivada parcial da perda Gamma em relação à predição com $\log$ como função de ligação}
    \frac{\partial \Loss}{\partial z_j} = 1 - \frac{y_j}{\hat{y}_j}%
...
\begin{equacaodestaque}{Derivada parcial da perda de Tweedie em relação à predição}
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \midrule
            Erro quadrático médio (MSE) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, estritamente convexa, baixa interpretabilidade dos valores numéricos (solução: utilizar a RMSE em conjunto como métrica), \\
            \addlinespace
            Erro absoluto médio (MAE) & Contínua ($C^0$), robusta a \textit{outliers}, convexa, boa interpretabilidade,\\
            \addlinespace
