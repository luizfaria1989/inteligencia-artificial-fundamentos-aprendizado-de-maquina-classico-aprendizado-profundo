...
A terceira seção explica a perda $\epsilon$-insensível, a função de perda para máquinas de vetores de suporte de regressão. A última seção aborda três funções de perda para outras distribuições de dados, são elas: a perda de Poisson, perda Gamma e perda de Tweedie. Por fim, o conteúdo é resumido em uma tabela, destacando as principais características das perdas apresentadas neste capítulo.
...
\begin{equacaodestaque}{Gradiente da perda log-cosh em relação à predição}
    \nabla_{\hat{y}}\mathcal{L}_{\text{log-cosh}} = \tanh(y - \hat{y})%
...
\subsection{Perda de Poisson (Poisson deviance)}%
\index{Funções de Perda!Perda de Poisson (\textit{Poisson deviance})}%
...
Como nosso intuito é minimizar a perda, devemos multiplicá-la por $-1$. Feito isso, encontramos que a perda de Poisson é dada pela Equação~\ref{eq:poisson-loss}. Neste caso, o termo $\lambda$ é dado pelo valor predito pelo modelo $\hat{y}$.
...
Como o valor real $y$ não é controlado pelo modelo, conforme ele for variando de iteração para iteração, vão existir diferentes gráficos de perda, isso está evidenciado na Figura~\ref{fig:poisson-comparison}. Quando o valor real é pequeno, a perda de Poisson possui uma convexidade mais ``apertada''. Já quando o valor real é grande, isso resulta em uma função mais ``larga''.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualização da derivada da perda de Poisson para diferentes valores de $y$.}%
...
\begin{equacaodestaque}{Gradiente da perda de Poisson em relação à predição com exp como função de conexão}
...
Para encontramos a perda Gamma, devemos derivá-la da FDP da sua respectiva distribuição. O primeiro passo é considerar uma parametrização diferente para escrever a FDP, queremos ter a função em termos da média esperada $\hat{y}$. Sabemos que para a parametrização da Equação~\ref{eq:gamma-distribution-pdf} a média é calculada por
...
Para finalizar, queremos que a perda seja zero quando a predição for perfeita ($y = \hat{y}$). Se substituirmos $\hat{y}$ por $y$ na equação anterior teremos $\ln y + 1$. Então subtraímos esse valor do numerador da fórmula, obtemos a Equação~\ref{eq:gamma-loss} para calcular a perda Gamma.
...
\begin{figure}[h!]
    ...
    \caption{Visualização da perda Gamma para diferentes valores reais $y$.}%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda Gamma é uma função de classe $C^{\infty}$, \textbf{infinitamente diferenciável} (ver Apêndice~\ref{ap:deducoes-gamma-loss}). Essa propriedade garante para a perda a possibilidade de ser utilizada em conjunto com otimizadores baseados em gradiente sem ocorrer muitos problemas, dado que as suas derivadas também serão contínuas.
    \item[Convexidade:] A perda Gamma \textbf{não é convexa} em relação à predição $\hat{y}$ globalmente, tornando-se côncava para grandes superestimativas (ver Apêndice~\ref{ap:deducoes-gamma-loss}). Mais precisamente, existe uma mudança de curvatura quando a predição excede o dobro do alvo, indicando que a otimização direta em $\hat{y}$ pode ser instável para métodos de segunda ordem (como Newton-Raphson).
    \item[Robustez:] A perda Gamma não é Lipschitz-contínua devido à singularidade na origem, portanto, \textbf{não é robusta a \textit{outliers}} (ver Apêndice~\ref{ap:deducoes-gamma-loss}). Contudo, o seu gradiente tende a zero no infinito, de forma que ela é robusta para erros onde $\hat{y} \gg y$.
...
Uma das formas de encontrar a derivada da perda Gamma é aplicar as técnicas tradicionais de derivação na função de perda, encontrando a Equação~\ref{eq:gamma-loss-derivada}.

\begin{equacaodestaque}{Gradiente da perda Gamma em relação à predição}
    \nabla_{\hat{y}}\mathcal{L}_{\text{gamma}} (y, \hat{y}) = \frac{1}{\hat{y}} -\frac{y}{\hat{y}^2} = \frac{\hat{y} -y}{\hat{y}^2}%
    \label{eq:gamma-loss-derivada}
\end{equacaodestaque}

Entretanto, também podemos considerar a derivação para o cenário em que a perda Gamma é aplicada em conjunto com a função logaritmo, neste caso, temos a Equação~\ref{eq:gradiente-gamma-link}. A derivada, é simplesmente a diferença relativa entre 1 e a razão entre o valor real $y$ e o previsto $\hat{y}$


\begin{equacaodestaque}{Gradiente da perda Gamma em relação à predição com $\log$ como função de ligação}
...
Da distribuição de Tweedie derivamos a sua perda, vista na Equação~\ref{eq:tweedie-loss}\footnote{Perceba que neste caso, não derivamos a perda de Tweedie de sua FDP nem de sua log-verossimilhança. Isso acontece porque a FDP da distribuição de Tweedie não apresenta uma fórmula fechada simples, como nos outros dois cenários anteriores.}. A perda de Tweedie une as outras duas distribuições que vimos anteriormente, pois dela, podemos escrever a perda Gamma e também a perda de Poisson com uma simples mudança no índice do parâmetro $p$.

\begin{equacaodestaque}{Perda de Tweedie}
...
\begin{figure}[h!]
    ...
    \caption{Visualização da perda Tweedie ($p=1.5$) para diferentes valores reais $y$.}%
...
Nessa seção será analisado os casos em que $1 < p < 2$ para a perda de Tweedie, configurando-a para a distribuição Poisson-Gamma. Para a situação em que $p = 1$ (perda de Poisson), veja a Seção~\ref{sec:poisson-loss}. Já para a situação em que $p = 2$ (perda Gamma), veja a Seção~\ref{sec:gamma-loss}.

\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda de Tweedie pertence à classe $C^\infty$, \textbf{infinitamente diferenciável}, no domínio $\hat{y} \in (0, \infty)$ para $p \in (1, 2)$ (ver Apêndice~\ref{ap:deducoes-tweedie-loss}). Dessa forma, as derivadas parciais existem e são contínuas, evitando o uso de técnicas como o subgradiente para poder calcular as derivadas.
    \item[Convexidade:] A perda de Tweedie \textbf{não é globalmente convexa} em relação a $\hat{y}$ para $1 < p < 2$ (ver Apêndice~\ref{ap:deducoes-tweedie-loss}).  A função é convexa apenas localmente, quando a predição não está muito longe do alvo. Se o modelo superestimar grosseiramente o valor ($\hat{y} > \frac{p}{p-1}y$), a função entra em uma região côncava.
    \item[Robustez:] A perda de Tweedie não é Lipschitz-contínua, portanto, \textbf{Não é robusta a \textit{outliers}} (ver Apêndice~\ref{ap:deducoes-tweedie-loss}). Contudo, assim como a perda Gamma, quando o valor da predição $\hat{y} \to \infty$ temos que o gradiente tende a zero. Indicando que superestimações não serão fortemente penalizadas.
    \item[$\log$ como função de ligação]: Semelhante à perda Gamma, podemos utilizar a função logaritmo para lidar com o problema da não-convexidade da perda de Tweedie em $1 < p < 2$. Substituindo essa relação na Equação \ref{eq:tweedie-loss}, obtemos a perda em função do \textit{logit} $z$:
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualizações da derivada da Perda de Tweedie para $p=1.5$ no plano.}%
    \label{fig:tweedie-loss-derivada}
    \fonte{O autor (2025).}
\end{figure}

\subsubsection*{Aplicações da Perda de Tweedie}%
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \addlinespace
            Perda de Poisson (\textit{Poisson deviance}) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, convexa (e estritamente convexa se $y > 0$), utilizada quando os dados seguem a distribuição de Poisson, \\
            \addlinespace
            Perda Gamma (\textit{Gamma deviance}) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, não convexa (solução: usar $\log$ como função de ligação), utilizada quando os dados seguem a distribuição Gamma, \\
            \addlinespace
            Perda de Tweedie (\textit{Tweedie deviance}) & Infinitamente diferenciável ($C^{\infty}$), sensível a \textit{outliers}, não convexa (solução: usar $\log$ como função de ligação), utilizada para substituir as perdas de Poisson e Gamma, utilizada quando os dados seguem a distribuição Poisson-Gamma, \\
            \addlinespace
        \end{tabularx}
