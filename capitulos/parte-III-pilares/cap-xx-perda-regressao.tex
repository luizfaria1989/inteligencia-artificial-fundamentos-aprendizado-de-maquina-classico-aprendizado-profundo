...
Escolher uma função que seja robusta a \textit{outliers} é uma escolha ideal para resolver problemas em que esse tipo de dado estará presente. Pois, os \textit{outliers} não irão conseguir afetar drasticamente o cálculo da perda.
...
A Figura \ref{fig:mse} ilustra o comportamento do erro quadrático médio. O gráfico da esquerda (a) mostra a vista em duas dimensões do \textit{MSE} com foco no erro, enquanto o gráfico da direita (b) apresenta a superfície no espaço dessa função de perda. As funções de perda neste capítulo estão em sua maioria representadas dessa forma, uma vez que apresentar a vista bidimensional com relação ao erro ilustra como o erro cresce. Enquanto a vista em três dimensões garante uma melhor visualização de como a função de perda é representada de fato.
...
\begin{itemize}
    ...
    \item \textbf{Sensibilidade a \textit{outliers}:} \textcite{LossesArticle} apontam que o erro quadrático médio possui a tendência de penalizar em maior força os erros que são originários de uma distância muito grande. O \textit{MSE} calcula a distância entre os pontos e depois eleva ela ao quadrado. Se essa distância for muito grande, o erro será maior ainda. Como consequência, se existe um conjunto grande de pontos que ficam fora da regressão, os \textit{outliers}, a função de perda irá constantemente retornar valores altos. Isso pode enviesar o aprendizado, dificultando a otimização do modelo. Mas, também pode ser uma vantagem em cenários em que os erros devem ser fortemente penalizados;
    \item \textbf{Convexa (nas predições):} A Figura~\ref{fig:mse} também mostra que o \textit{MSE} é uma função convexa. O seu gráfico tem a típica forma de um funil, apresentando um único ponto de mínimo global. A convexidade ajuda na otimização dessa função, caso se esteja usando um otimizador baseado no gradiente descendente. Contudo, \textcite{LossesArticle} destacam que em redes neurais profundas essa função pode se tornar não-convexa, devido às transformações não-lineares que são feitas ao construir o modelo;
    ...
    \item \textbf{Continuidade:} Os gráficos da Figura~\ref{fig:mse} mostram que o \textit{MSE} é uma função contínua em todo o seu domínio. Portanto, ao derivá-lo, não serão encontrados nenhum problema de descontinuidade.
\end{itemize}
...
Cabe também discutir a \textbf{raiz do erro quadrático médio}, também chamada de \textbf{\textit{root mean squared error}} (\textbf{\textit{RMSE}}). Ela está na Equação~\ref{eq:rmse-metric}. Ela é uma função derivada do \textit{MSE}, cuja diferença está no cálculo de uma raiz quadrada a partir do resultado do \textit{MSE}.
...
O erro quadrático médio é ideal para ser utilizado em cenários que deve-se garantir uma maior penalização dos erros do modelo que durante seu treinamento. Isso acontece devido à sua sensibilidade a \textit{outliers}. Contudo, vão existir cenários em que essa propriedade pode ser uma desvantagem, enviesando o treinamento de um modelo de regressão. Diante disso, alternativas como o erro absoluto médio tornam-se mais viáveis.

\subsection{Erro absoluto médio (MAE)}%
\index{Funções de Perda!Erro absoluto médio (MAE)}%
\label{sec:mae-loss}

O \textbf{\gls{mae-loss}}, também chamado de \textbf{perda L1}, é uma função que tem o mesmo propósito do erro quadrático médio, ser utilizada para tarefas de regressão. Neste caso, o \textit{MAE} não possui uma origem definida que como o \textit{MSE}, esse conceito de minimizar a diferença de um resultado pelo seu valor real já havia sendo utilizado a bastante tempo. Contudo, trabalhos como \textit{Greedy function approximation: A gradient boosting machine} de \textcite{GreedyFunctionApproximation} fazem uso do erro absoluto médio para resolver problemas de aprendizado de máquina. No texto, o autor desenvolve um algoritmo de \textit{boosting} específico para o \textit{MAE}, neste caso, essa função é apresentada com outro nome, \textbf{\textit{Least-Absolute-Deviation}} (\textbf{\textit{LAD}}), a qual é responsável por dar nome ao algoritmo criado, o \texttt{LAD\_TreeBoost} \parencite{GreedyFunctionApproximation}. \textcite{GreedyFunctionApproximation} explica que esse algoritmo faz uso de uma árvore de regressão com a perda, de forma que para prever a pseudo-resposta que é o sinal dos resíduos atuais é utilizado o método dos mínimos quadrados. Como consequência, o modelo é atualizado adicionando em cada nó terminal da nova árvore criada a mediana dos resíduos daquela região específica.

Um trabalho mais recente que explora o uso dessa função é o \textit{Image-to-Image Translation with Conditional Adversarial Networks} \parencite{ImageToImage}. Nele, \textcite{ImageToImage} argumentam que preferiram trabalhar com a função de perda \textit{L1 distance} (um dos diferentes nomes utilizados para se referir ao \textit{MAE}) devido à essa função gerar imagens menos borradas. Essa comparação das funções de perda utilizadas na pesquisa está nas imagens da Figura~\ref{fig:comparativo-perdas-image-to-image}. A perda que apresenta os resultados mais consistentes com a realidade é justamente a \textit{L1 + cGAN}, a qual possuí o \textit{MAE} em sua composição.
...
Voltando para a analogia dos dardos. Consideramos que será feito um novo jogo de dardos com dez participantes, em que cada um tem direito de jogar apenas um dardo. No final, os dez dardos serão somados e com base neles, será dado o resultado da equipe. Se tivéssemos uma equipe muito ruim utilizando como métrica o \textit{MSE} para avaliar os pontos, o resultado seria péssimo, porque esses dardos estariam muito longes do centro, e como o erro é elevado ao quadrado, isso geraria uma pontuação muito baixa. 

Contudo, se a métrica escolhida para avaliar o jogo fosse o \textit{MAE} o resultado seria bem melhor. Visto que, o erro cresce de forma linear. Saindo da analogia e voltando para o cenário de aprendizado de máquina, os jogadores ruins se comparam com os \textit{outliers}, eles estão presentes nos dados e vão atrapalhar o aprendizado do seu modelo. Entretanto, utilizar uma função que não seja tão sensível na medição desses \textit{outliers} pode ser uma boa alternativa para trabalhar em cenários em que essa configuração será muito comum.

O \textit{MAE} está definido na Equação~\ref{eq:mae}. Ele é responsável por calcular a diferença entre os dois pontos, o valor real $\hat{y}_j$ e o valor predito $y_j$, para todos os $N$ casos analisados e a partir disso calcular a média dos resultados.
...
Os gráficos do erro absoluto médio estão na Figura~\ref{fig:mae}. Seguindo a mesma lógica: na subfigura à esquerda, está a visão em duas dimensões, e na subfigura à direita, a superfície no espaço.
...
\begin{itemize}
    \item \textbf{Não-negatividade:} Como é apontado nos gráficos da Figura~\ref{fig:mae}, o erro absoluto médio compartilha da não negatividade assim como o \textit{MSE}. A sua saída saída será sempre positiva ou zero, independente do valor de entrada. Isso se dá, devido à propriedade do módulo, que não admite números negativos para sua saída;
    \item \textbf{Robustez para \textit{outliers}}: Como \textcite{LossesArticle} explicam, o \textit{MAE} não penaliza os \textit{outliers} de forma tão severa como o erro quadrático médio. Diferente do \textit{MSE}, em que o erro cresce de forma quadrática, no \textit{MAE}, ele cresce de forma linear. Significando que diferente do \textit{MSE} que era sensível aos \textit{outliers}, o \textit{MAE} também reage a eles, mas de forma menos severa;
    \item \textbf{Convexa (nas predições):} Voltando para o seu gráfico, o erro absoluto médio segue a mesma forma de funil do erro quadrático médio, podendo ser considerado uma função convexa e com um só ponto de mínimo global. Contudo, assim como no \textit{MSE}, \textcite{LossesArticle} advertem que para modelos de aprendizado profundo, o \textit{MAE} pode deixar de ser uma função convexa devido às muitas camadas e funções não-lineares;
    \item \textbf{Não-derivável em zero:} Esse ponto acontece justamente devido à forma que a função apresenta, ela faz uma espécie de ``bico'' em zero. Além disso, ao calcular os seus limites laterais para verificar a continuidade da função é nota-se que eles apresentam valores diferentes;
...
Como foi destacado anteriormente, o \textit{MAE} não é derivável em zero. Inicialmente pode ter-se a ideia de que devido a isso, essa não seja uma função boa para ser utilizada em conjunto com otimizadores baseados em gradiente. Porque, pontos de descontinuidade como este podem acabar afetando a forma como o modelo é otimizado. Contudo, \textcite{LossesArticle} argumentam, é possível resolver esse problema utilizado técnicas de subgradiente, sendo possível então escrever a derivada do \textit{MAE} com a Equação~\ref{eq:mae-derivada}.
...
\begin{figure}[h!]
    ...
    \caption{Visualizações da derivada (gradiente) da função de perda erro absoluto médio.}%
    \label{fig:mae-derivada}
    \fonte{O autor (2025).}
\end{figure}

A derivada do erro absoluto médio é representada em forma de uma função por partes. Ela é divida em duas diferentes retas, parecida com a função de ativação degrau unitário. Ademais, existe um ``bico'' na união dessas duas retas, que acontece devido à descontinuidade dessa função em zero, impedindo-a de ser derivada nesse ponto. A derivada do \textit{MAE} pode ser interpretada como a composição de duas retas constantes, a primeira constante em -1, e a segunda constante em 1.
...
Além dos casos discutidos no início da seção: o \texttt{LAD\_TreeBoost} e o artigo \textit{Image-to-Image Translation with Conditional Adversarial Networks}, o \textit{MAE} está presente em uma série de trabalhos. Atuando tanto como função de perda, quanto servindo como uma métrica de avaliação, indicando se o modelo desenvolvido está performando bem ou não. 
...
A perda de Huber, representada com as notações deste livro, está na Equação~\ref{eq:huber-loss}.
...
Os gráficos da perda de Huber estão na Figura~\ref{fig:huber-loss}. Dessa forma, na Figura~\ref{fig:huber-2d} representa a visualização em duas dimensões dessa função, enquanto na Figura~\ref{fig:huber-3d} representa no espaço da superfície, assim como o que vem sendo feito até agora. A perda de Huber é como se fosse duas funções em uma, até um certo ponto do gráfico ela tem o comportamento de uma função quadrática. Contudo, após passar do limite de $\delta$, ela passa a ser uma função linear.
...
Um ponto a ser destacado ao utilizar a perda de Huber é com relação a escolha de valores para o parâmetro $\delta$. Um valor muito pequeno para $\delta$ faz com que a função se comporte mais como o erro absoluto médio, essa situação está presente na Figura~\ref{fig:huber-comparacoes-mae}. Já ao escolher um valor muito grande para $\delta$, a perda de Huber passa a se assemelhar mais à função erro quadrático médio, como mostra a Figura~\ref{fig:huber-comparacoes-mse}. \textcite{LossesArticle} explicam que a escolha de valores para $\delta$ pode ser feita de forma empírica, através de validação cruzada (\textit{cross-validation}). Logo, testes são recomendados a fim de escolher o melhor valor para $\delta$ no cenário em que está sendo trabalhado.
...
\begin{itemize}
    \item \textbf{Robustez a \textit{outliers}:} Assim como o \textit{MAE}, a perda de Huber não penaliza de forma quadrática os erros como comparado com o erro quadrático médio, portanto, os \textit{outliers} não conseguem afetar drasticamente o cálculo da perda dependendo do valor de $\delta$ escolhido \parencite{LossesArticle};
...
Como explicam \textcite{LossesArticle}, o gradiente da perda de Huber deve ser calculado por partes, sendo que é possível utilizar a Equação~\ref{eq:huber-loss-derivada} como guia.

\begin{equacaodestaque}{Derivada Parcial da Perda de Huber (Huber \textit{loss}) em Relação à $\hat{y}$}
...
Os gráficos da derivada parcial da perda de Huber em relação à predição do modelo estão na Figura~\ref{fig:huber-derivada}. Na Figura~\ref{fig:huber-derivada-2d} está a representação no plano, já na Figura~\ref{fig:huber-derivada-3d} está a representação da superfície no espaço.
...
Perceba que dá para ver como os pontos de descontinuidade atrapalham o formato do gráfico. Existem dois pontos que formam um ``bico'', eles são justamente os pontos em que $y - \hat{y} = \delta$. A descontinuidade pode não atrapalhar a plotagem do gráfico de forma geral, contudo, é sempre importante destacar os detalhes em que esses pontos refletem no gráfico da derivada.
...
A perda de Huber consegue unir o melhor dos dois mundos quando se trata das perdas \textit{MSE} e \textit{MAE}. É uma função que atua de forma quadrática até certo ponto e depois, de forma linear. De forma que ela garante ser robusta a \textit{outliers}, uma excelente vantagem para essa função. Entretanto, ao utilizar um parâmetro $\delta$ para definir o comportamento da função, a perda de Huber deixa de ser contínua em todo seu domínio. Esse fato pode ser um empecilho quando se trata se otimizadores baseados em gradiente. Assim, uma solução para contornar esse problema é a perda log-cosh.
...
A fórmula da perda log-cosh está na Equação~\ref{eq:log-cosh-loss}. Ela não adiciona nenhuma função nova, ela apenas faz a aplicação da função logaritmo que recebe como parâmetro de entrada a função cosseno hiperbólica. Essa combinação gera uma séries de propriedades interessantes, as quais serão discutidas depois de analisar o seu gráfico.
...
\begin{itemize}
    \item \textbf{Convexa:} Como os gráficos da Figura~\ref{fig:log-cosh-loss} apontam, a perda log-cosh é uma função convexa. Apresentando um formato característico de funil, além de possuir um único ponto de mínimo global. Contudo, não é possível garantir essa propriedade em cenários em que ela está sendo aplicada em modelos de redes neurais densas, devido às transformações não-lineares que ocorrem;
    \item \textbf{Robustez a \textit{outliers}:} Assim como a perda de Huber e o erro absoluto médio, a perda log-cosh não pune de forma agressiva os erros cometidos pelo modelo ao ser treinado. Isso garante que essa função possa ser aplicada em cenários em que os dados possuem muitos \textit{outliers} sem afetar drasticamente o treinamento do modelo;
    \item \textbf{Continuidade e diferenciabilidade:} Diferente de a perda de Huber, que possui pontos de descontinuidade na ligação da função linear com a quadrática, a perda log-cosh consegue ser contínua em todos os seus pontos. Além disso, isso também é uma vantagem sobre o erro absoluto médio, pois este também apresenta um ponto de descontinuidade em 0, o qual precisa do cálculo do subgradiente para garantir o aprendizado dos modelos que fazem uso de otimizadores baseados em gradiente. Portanto, além de ser contínua, a perda log-cosh pode ser derivada em todos os seus pontos. Algo útil caso esteja sendo usados otimizadores que atuam com o uso do cálculo do gradiente.
\end{itemize}
...
A derivada da perda log-cosh está na Equação~\ref{eq:log-cosh-derivada}. Ela envolve o cálculo da tangente hiperbólica, que coincidentemente, também é utilizada em aprendizado de máquina como uma função de ativação, tendo como objetivo introduzir a não-linearidade para as saídas de uma camada densa.
...
Além disso, a Figura~\ref{eq:log-cosh-derivada} contém as representações gráficas da derivada da perda log-cosh para o caso em que ela é calculada em relação à predição. Caso você leitor tenha lido o Capítulo~\ref{cap:ativacao-sigmoidais}, você não verá nada novo aqui, é apenas o gráfico característico em formato de ``S'' que as funções sigmoidais possuem. Um ponto interessante a ser destacado na figura, é que as saídas dessa função serão em um intervalo $[-1, 1]$. Isso faz que podem fazer com que o sinal do gradiente fique alternando, garantindo uma convergência mais rápida em alguns casos.
...
Além das aplicações discutidas no início da seção sobre a perda Log-cosh, ela também está presente em várias outras pesquisas. Com esse intuito, essa seção explica algumas aplicações dessa função de perda para resolver problemas no contexto de aprendizado de máquina.
...
Com as quatro funções vistas até este momento, é possível resolver a grande maioria dos problemas de regressão. Contudo, vão existir cenários em que o objetivo será lidar com o erro relativo. Garantindo uma melhor análise se o modelo treinado está subestimando ou superestimando os rótulos nas suas predições. Uma das formas de lidar com modelos que estão subestimando os dados é utilizar o erro quadrático logarítmico médio como função de perda.
...
Para fazer o trabalho dessa regressão foi utilizada a função de perda erro quadrático médio. Essa função calcula primeiro a diferença entre o valor previsto pelo modelo e o valor apresentado no rótulo. Com isso, essa diferença será a mesma para esses dois cenários, 50.000 R\$.
...
\begin{itemize}
    ...
    \item \textbf{Tendência de penalizar subestimações:} Como mostram os gráficos da Figura~\ref{fig:msle-loss}, a função \textit{MSLE} não forma uma curva convexa simétrica verticalmente, conforme os valores vão diminuindo, a perda aumenta consideravelmente. Enquanto isso, conforme os valores aumentam, a perda também aumenta, mas não de forma tão agressiva quanto no sentido inverso. Isso significa que quanto maior for a diferença entre o valor previsto $\hat{y}_j$ e o valor real $y_j$, existe uma tendência de que a perda será maior;
    \item \textbf{Continuidade e Diferenciabilidade:} Ainda com relação aos seus gráficos é possível analisar a continuidade dessa função. Note que ela não apresenta nenhum ponto que atrapalhe a diferenciabilidade que poderia afetar o cálculo das derivadas, e, com isso atrapalhar o fluxo do gradiente. Além disso, o uso dos logaritmos para essa função permite que suas curvas sejam suaves, o que também ajuda na sua continuidade.
\end{itemize}
...
Na equação~\ref{eq:msle-derivada} está a derivada do erro quadrático médio logarítmico em relação os valores preditos pelo modelo $\hat{y}_j$. Ela será útil para a retropropagação do modelo.
...
Em seguida, está a representação gráfica da derivada da Equação~\ref{eq:msle-derivada} na Figura~\ref{fig:msle-derivada}. Seguindo a mesma regra das outras figuras apresentadas até agora, na Figura~\ref{fig:msle-derivada-2d}, à esquerda, está a representação em duas dimensões, enquanto na Figura~\ref{fig:msle-derivada-3d}, à direita, está a representação da superfície no espaço.
...
\begin{figure}[h!]
    ...
    \caption{Visualizações da derivada (gradiente) da função de perda erro quadrático médio logarítmico.}%
    \label{fig:msle-derivada}
    \fonte{O autor (2025).}
\end{figure}

O uso dos logaritmos na função, fazem com que a superfície do \textit{MSLE} tenha uma superfície suave e contínua. Isso também é refletido na sua derivada. Ela é de grande utilidade para os algoritmos de otimização que trabalham com o gradiente, de forma que eles não precisam ter que escapar de pontos de descontinuidade, como ocorriam em outras funções anteriores, como a perda de Huber e o \textit{MAE}.
...
Além disso, voltando para o exemplo anterior dos modelos prevendo preços de imóveis. Cabe discutir como o \textit{MSLE} se compara ao erro quadrático médio, e como ele pode ser uma alternativa para função de perda em cenários em que é preciso garantir que as subestimações sejam mais penalizadas.
...
Ao calcular a raiz quadrada do erro quadrático médio logarítmico é possível ter uma noção melhor da magnitude dos erros, dado que eles são mascarados ao serem elevados ao quadrado pela expressão do \textit{RMSE}. Diante disso, a equação da raiz do erro quadrático médio logarítmico consegue melhorar a interpretabilidade dos erros, servindo como uma métrica, a qual pode ser aplicada para problemas em que as subestimações devem ser mais penalizadas.
...
Ao calcular a média dos erros, está sendo presumido que a tendência central é a característica mais informativa daquela distribuição de dados, seguindo a clássica distribuição Gaussiana. Contudo, e quando isso não for a realidade? E quando a caraterística mais comum estiver deslocada mais para à esquerda (como na Figura~\ref{fig:dist-skew-esquerda}), ou mais para à direita da distribuição (como na Figura~\ref{fig:dist-skew-direita})?
...
Existe uma função de perda que consegue lidar justamente com esse cenário, em que os dados seguem uma distribuição assimétrica. Ela é a perda quantílica, que foca em estudar como a perda acontece em diferentes quantis da distribuição. Ao utilizá-la, é definido o quantil que será analisado, de forma que é possível escolher se ela irá funcionar de forma assimétrica, ou de forma parecida com o \textit{MAE}.

Também há uma outra função de perda que também não trabalha com o cálculo da média dos erros, ela é a $\epsilon$-insensível. Sendo uma função essencial para a tarefas de regressão para máquinas de vetores de suporte. Neste caso, ao utilizar essa função, é definido uma ``margem de tolerância'' em que caso o erro esteja dentro dessa média, a perda não irá penalizar o modelo.

\subsection{Perda quantílica (quantile loss)}%
\index{Funções de Perda!Perda quantílica (\textit{quantile loss})}

No artigo \textit{Regression Quantiles} dos autores \textcite{regression-quantiles} estava sendo analisada uma nova técnica de regressão que se baseava nos quantis de uma distribuição. Como motivação para esse estudo, os autores discutem a ideia de que ``todos acreditam a lei de Gauss para os erros, porque eles acreditam que é um teorema matemático, e os matemáticos acreditam porque é um fato experimental'' \parencite{regression-quantiles}. Contudo, isso não é uma verdade absoluta, existem casos, como os vistos na introdução dessa seção que fogem da distribuição normal. Como consequência, técnicas mais conhecidas, como o \textit{MSE} para o cálculo dos erros com base na média, não são ideais para minimizar o erro de um modelo.
...
A partir dessa expressão, cabe analisar os casos, considerando a assimetria da perda.
...
\begin{itemize}
    ...
    \item \textbf{Equação base:} A Equação~\ref{eq:soma-das-perdas-assimetricas} mostra que nesse caso é aplicado o termo $\tau|y_j - \hat{y}_j|$
    \item \textbf{Dedução:}
          \begin{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Equação base:} Voltando para a Equação~\ref{eq:soma-das-perdas-assimetricas}, mostra que nesse caso é aplicado o termo $(1 - \tau)|y_j - \hat{y}_j|$
    \item \textbf{Dedução:}
          \begin{enumerate}
...
Tendo esses dois casos, deriva-se a definição por partes da \textbf{perda quantílica}, também conhecida como \textbf{\textit{quantile loss}}. A expressão dessa função é dada pela Equação~\ref{eq:quantile-loss}. Neste caso, está sendo considerado que a situação $\hat{y} = y$ faz parte do primeiro caso estudado.
...
Além dessa expressão, existe também uma versão compacta utilizando a função $\max$ para definir a função por partes da \textit{quantile loss} em apenas uma linha. Essa versão está presente na Equação~\ref{eq:quantile-loss-com-max}.

\begin{equacaodestaque}{Perda quantílica reescrita com a função $\max$}
...
A Figura~\ref{fig:quantile-loss-tau05} mostra a representação em duas dimensões da perda quantílica, enquanto a Figura~\ref{fig:quantile-loss-tau05} mostra a superfície da perda quantílica. Ambas mostram um cenário em que o valor de tau é dado por 0.5.
...
Nessas figuras, a função é idêntica ao erro absoluto médio, o gráfico tem o formato em ``V'' característico dessa função. Isso acontece porque ao considerar calcular a perda quantílica para o quantil 50\% (indicado por 0.5), está sendo calculado a média dos dados, algo que também é feito utilizando o \textit{MAE} como função de perda.

Mas esse não é o cenário ideal para se utilizar a perda quantílica. O diferencial dessa função é quando ela precisa ser utilizada em cenários em que a distribuição de dados é irregular, e não segue a distribuição normal. Essa função tem uma propriedade que pode ser ajustada para penalizar de forma assimétrica os erros que um modelo comete. 
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Gráfico da derivada da Perda Quantílica (em Relação ào erro).}%
