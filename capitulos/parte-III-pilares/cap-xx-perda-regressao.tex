...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}]
    ...
    \item[Não-negatividade:] omo gráficos da Figura~\ref{fig:mse} ilustram, o \textit{MSE} é retorna apenas valores positivos. Isso se dá devido à diferença das entradas estar sendo calculada, e, em seguida, elevada ao quadrado, impendido que valores negativos ocorram na saída.
    \item[Baixa interpretabilidade:] O \textit{MSE} eleva o erro ao quadrado. Ao retornar o valor da perda, não é visto diretamente a distância entre os pontos reais e os pontos preditos pelo modelo. Essa cálculo dificulta saber, de forma rápida, quão bem ou mal o modelo está performando. Funções como a \textit{MAE}, em que o erro é dado pelo módulo da distância entre os dois pontos são mais diretas em mostrar a performance do modelo;
...
\subsubsection*{Raiz do erro quadrático médio (RSMLE)}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Estimação de custos médicos (saúde):] Para fazer previsões de custos médicos \textcite{MedicalCostsEstimationUsingLR} utilizam técnicas de regressão, para avaliar os modelos criados, os autores utilizam o \textit{MSE}, \textit{MAE} e $R^2$ como métricas.
    \item[Estimação de preços de imóveis (mercado imobiliário):] \textcite{OptimalHousePricePrediction} criam modelos de regressão (como regressão linear, florestas aleatórias e XGBoost) com intuito de prever valores de casas, eles fazem uso do \textit{MSE} e da \textit{RMSE} para avaliar o desempenho.
    \item[Previsão da produção agrícola (agronomia):] \textcite{CouplingMachineLearningAndCropModeling} combinam aprendizado de máquina com modelagem de culturas para prever a produção das plantações na região do cinturão do milho nos Estados Unidos. Para avaliar as predições, eles utilizam a \textit{RMSE} como métrica \parencite{CouplingMachineLearningAndCropModeling}
    \item[Previsão de demanda de energia (gestão energética):] \textcite{OptimizingFL} fazem uma adaptação do \textit{MSE} transformando-o no \textit{Exponentially Weighted Mean Squared Error} (\textit{EW-RSM}), essa adaptação é justificada como uma forma de enfatizar a acurácia em previsões de longo prazo atribuindo pesos exponencialmente crescentes aos erros em etapas de tempo posteriores.
\end{description}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Boa interpretabilidade:] Diferente do erro quadrático médio em que o erro total é elevado ao quadrado, no \textit{MAE} isso não ocorre. O erro é simplesmente a média das diferenças dos pontos. É intuitivo interpretar os resultados que essa função de perda retorna, considerando que não é preciso fazer nenhum cálculo adicional para ter uma noção precisa se as previsões feitas pelo modelo condizentes ou não dos valores reais.
\end{description}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Avaliação de idade óssea e estimação do escore de cálcio na artéria coronária (Saúde):] \textcite{chao2022regressionmetriclosslearning} desenvolvem algoritmos de regressão para estimar escore de cálcio da artéria coronária além de um segundo algoritmo para avaliação da idade óssea. Como forma de avaliar as previsões \textcite{chao2022regressionmetriclosslearning} utilizam o \textit{MAE} em uma das métricas.
    \item[Restauração de imagens (Engenharia):] \textcite{Noise2Noise} estavam buscando uma maneira ideal de remover textos de imagens, de forma que o \textit{MAE}, por ser uma função de perda robusta, conseguiu atingir bons resultados nessa tarefa.
    \item[Previsão da produção de energia eólica (Setor energético):] \textcite{MinimumOpenDataSubsetForWindPowerPrediction} utilizam um modelo de florestas aleatórias para prever a produção de energia eólica. Os autores avaliam o desempenho com as funções \textit{MAE}  e \textit{RMSE} \parencite{MinimumOpenDataSubsetForWindPowerPrediction}.
    \item[Previsão de poluição do ar (Setor ambiental):] \textcite{nedungadi2025aircastimprovingairpollution} buscam formas de melhorar a previsão da poluição do ar, os pesquisadores utilizam uma função inspirada pelo \textit{MAE}, o \textit{Frequency-weighted Mean Absolute Error} (\textit{fMAE}), tendo como principal vantagem lidar com variáveis que apresentam uma distribuição de cauda pesada.
\end{description}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Previsão de índices do mercado financeiro (Economia):] \textcite{FinantialMarketForecastingUsingRNN} criam algoritmos de aprendizado de máquina prever índices de ações globais (como o FTSE 100, S\&P 500 e HSI), construindo diferentes modelos, como uma rede neural recorrente. Eles utilizam a perda Log-Cosh junto com o \textit{MAE}, o \textit{MSE}, a \textit{RMSE} e também a perda de Huber \parencite{FinantialMarketForecastingUsingRNN}.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] O \textit{MSLE} é uma função de classe $C^{\infty}$ (infinitamente diferenciável) (ver Apêndice~\ref{ap:deducoes-msle}). Isso contribui para uma função de perda que pode ser facilmente utilizada em conjunto com otimizadores baseados em gradiente, dado que não serão encontrados problemas de continuidade ao derivar essa função.
    \item[Convexidade:] JA função \textit{MSLE} é convexa em relação à predição transformada $z = \log(1+\hat{y})$, mas não é globalmente convexa em relação a $\hat{y}$ (ver Apêndice~\ref{ap:deducoes-msle}). 
    \item[Robustez:] A função \textit{MSLE} é Lipschitz-contínua, portanto é robusta a \textit{outliers} (ver Apêndice~\ref{ap:deducoes-msle}). Diferente do erro absoluto médio, que quando o valor do erro ``explode'' para infinito o seu gradiente será 1, no \textit{MSLE} o seu gradiente tende a 0. Como resultado, para erros muito grandes o erro logarítmico médio gera uma correção irrisória, o que pode não ser útil em certos casos.
    \item[Foco no erro relativo (percentual):] Quando o \textit{MSLE} é utilizado como função de perda, diferente das outras vistas até agora que medem o erro absoluto, o erro quadrático logarítmico médio mede o erro percentual/relativo. Isso acontece por conta dos uso dos logaritmos dessa função, que são responsáveis por comprimir a escala dos dados. Portanto, erros em que o valor predito é 50\% menor que o real são mais penalizados em que cenários nos quais o valor predito é 5\% menor;
    \item[Tendência de penalizar subestimações:] Como aponta a Figura~\ref{fig:msle-loss}, o \textit{MSLE} não forma uma curva convexa simétrica verticalmente, conforme os valores vão diminuindo, a perda aumenta consideravelmente. Enquanto isso, conforme os valores aumentam, a perda também aumenta, mas não de forma tão agressiva quanto no sentido inverso. Isso significa que quanto maior for a diferença entre o valor previsto $\hat{y}_j$ e o valor real $y_j$, existe uma tendência de que a perda será maior;
    \item[Restrição de domínio (não-negatividade):] Como os logaritmos são indeterminados para valores nulos ou negativos, o \textit{MSLE} exige que os valores de entrada sejam positivos ($y_j > 0$ e $\hat{y}_j > 0$). Caso os conjunto de dados contenha valores negativos (como lucro/prejuízo ou temperaturas), o \textit{MSLE} irá causar erros matemáticos. Para contornar esse problema, deve-se aplicar uma transformação nos dados, como somar uma constante para deslocar os valores negativos para positivos, ou ser utilizada outra função de perda.
\end{description}
...
\subsubsection*{Raiz do erro quadrático médio logarítmico (RSMLE)}
...
Ao calcular a raiz quadrada do erro quadrático médio logarítmico tem-se uma noção melhor da magnitude dos erros, dado que eles são mascarados ao serem elevados ao quadrado pela expressão do \textit{RMSLE}. Diante disso, a equação da raiz do erro quadrático médio logarítmico consegue melhorar a interpretabilidade dos erros, servindo como uma métrica que pode ser aplicada para problemas em que as subestimações devem ser mais penalizadas.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Predição do preço de casas (Mercado imobiliário):] \textcite{SunKim-NetOverYourHead} desenvolvem uma rede neural para fazer a previsão do preço de casas. Eles utilizam como métrica o \textit{RMSLE}, como justificativa, os autores explicam que o \textit{dataset} que está sendo utilizado apresenta casas que têm ordem de magnitude de diferença de preço, e com isso uma previsão ruim para uma casa de alto valor terá mais peso que uma previsão ruim para uma casa de baixo valor; ao usar o \textit{RMSLE} é possível capturar as ordens de magnitude dos preços das casas, algo que não acontece com o \textit{RMSE} \parencite{SunKim-NetOverYourHead};
    \item[Predição no tempo de internação de pacientes (Saúde):] \textcite{Winter2023PredictingLOS} analisam maneiras de prever o tempo de internação de um paciente qualquer após receber alta no pronto-socorro e sua transferência para a próxima unidade hospitalar. Os autores usam um modelo de regressão de \textit{gradient boosting} com arquitetura \textit{CatBoost}, para a função de perda, os autores escolhem duas, a primeira sendo o \textit{RMSE} e a segunda o \textit{RMSLE}, como justificativa, os autores apontam que o \textit{RMSLE} penaliza erros proporcionais e é menos afetado por \textit{outliers};
    \item[Previsão de séries temporais (Área):] \textcite{Jadon2022ComprehensiveSurvey} analisam uma série de funções de perda que são comumente utilizadas na previsão de séries temporais, sendo uma delas o \textit{MSLE}. Os autores apontam que o \textit{MSLE} reduz o efeito punitivo de diferenças significativas em grandes valores previsos; sendo apropriado quando o modelo prevê quantidades não escalonadas diretamente \parencite{Jadon2022ComprehensiveSurvey}. \textcite{Jadon2022ComprehensiveSurvey} analisam também o \textit{RMSLE}, explicando que ele pode ser uma função de perda para ser utilizada em cenários em que a subestimação dos valores reais não é aceitável, mas a superestimações do modelo não são consideradas um problema.
\end{description}
...
O comportamento da derivada da perda quantílica também se assemelha com \textit{MAE}. Como a Figura~\ref{fig:quantile-loss-derivada} aponta, a sua derivada é uma função linear por partes, isso ajuda a entender também o porquê da perda quantílica ser robusta a \textit{outliers}. Independente dos valores que o erro pode alcançar, a perda nunca ira crescer descontroladamente para o infinito causando problemas de gradientes explosivos. Diferente do \textit{MAE} que é 1-Lipschitz-contínua, a perda quantílica tem seus limites modificados conforme o valor do hiperparâmetro $\tau$ é configurado, mas ainda assim, ela é Lipschitz-contínua.
...
A \textbf{perda epsilon-insensível}, também chamada \textbf{$\epsilon$-insensível} se relacionada diretamente com as máquinas de vetores de suporte, sendo essencial para possibilitar tarefas de regressão pelas \textit{SVMs}. Nesse sentido, o artigo \textit{Support Vector Regression Machines} dos pesquisadores \textcite{SupportVectorRegressionMachines} é um dos responsáveis por introduzir essa função de perda. Nele, os autores detalham um novo uso das máquinas de vetores desenvolvidas por Vapnik, as quais antes eram utilizadas para resolver problemas de classificação \parencite{SupportVectorRegressionMachines}.
...
Uma das aproximações para resolver esse problema é dada Equação~\ref{eq:svms-para-regressao} como explicam \textcite{SupportVectorRegressionMachines}.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Esparsidade:] A perda $\epsilon$-insensível promove a esparsidade no modelo que está sendo treinado. Pelo fato de existir uma faixa de valores do erro que a perda irá retornar zero como valor, essa função de perda ajuda a criar gradientes esparsos nos problemas de regressão de \textit{SVMs}.
\end{description}

\subsubsection*{Gradiente da perda epsilon-insensível}

Para encontrar o gradiente da perda $\epsilon$-insensível deve-se utilizar do subgradiente, assim como no \textit{MAE}. O subgradiente consegue lidar com os pontos de não diferenciabilidade (quando erro é igual a epsilon), ao aplicar a técnica chegamos na Equação~\ref{eq:epsilon-insensitive-derivada} para a derivada parcial em relação à predição.
