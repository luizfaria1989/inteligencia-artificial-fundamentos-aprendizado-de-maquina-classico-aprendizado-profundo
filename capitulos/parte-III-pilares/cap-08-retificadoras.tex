...
\begin{figure}{h!}
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2:2] {max(0, x)};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[h!] % Use [htbp] para dar flexibilidade ao LaTeX
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2:0] {0};
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2:2] {x > 0 ? x : 0.1*x};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2:0] {\alphaVal};
        \addplot[black, thick, domain=0:2] {1};
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \def\alphaVal{0.2} 
        \addplot[black, thick, domain=-2:2] {x >= 0 ? x : \alphaVal*x};
        \end{axis}
    \end{tikzpicture}
...
Um feito importante que deve ser destacado sobre a \textit{PReLU}, que inclusive é o nome do artigo que foi responsável por introduzir essa função para a comunidade científica, é de que durante a pesquisa do texto \textit{Delving Deep into Rectifiers: Surpassing Human Level Performance on Image Net Classification}, \textcite{PReLUArticle} foram capazes de criar uma rede neural capaz de superar a capacidade humana de reconhecer diferentes conjuntos de imagens no \textit{ImageNet} Classification, isso ocorreu porque a um humano ao analisar o \textit{ImageNet} apresenta uma taxa de erro \textit{Top-5} de 5.1\% e em um dos testes realizados pelos autores, uma rede neural alcançou uma taxa de erro \textit{Top-5} de 4.94\%, superando assim a capacidade humana de reconhecimento de imagens. 
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \def\alphaVal{1} 
        \addplot[black, thick, domain=-2:2] {x >= 0 ? x : \alphaVal*(exp(x) - 1)};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2.3:2.3, samples=100] {x > 0 ? 1 : \alphaVal*exp(x)};
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2:2, samples=100] {x > 0 ? \lambdaVal*x : \lambdaVal*\alphaVal*(exp(x) - 1)};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick, domain=-2:2, samples=100] {x > 0 ? \lambdaVal*1 : \lambdaVal*\alphaVal*exp(x)};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \addplot[black, thick] {x >= 0 ? x : ln(1+x)};
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{3}{>{\raggedright\arraybackslash}X}}
            ...
            \midrule
            \textit{Rectfied Linear Unit} (\textit{ReLU}) & Retorna $x_j$ quando $x_j > 0$, caso contrário, retorna zero. & É a função padrão para ser utilizada em uma rede \textit{feedforward}. & Sobre do problema dos \textit{ReLUs} agonizantes. \\
            \addlinespace
            ...
            \addlinespace
            \textit{Parametric ReLU} (\textit{PReLU}) & Variante da \textit{Leaky ReLU} em que $\alpha$ é um parâmetro aprendível & Por possuir um coeficiente aprendível, a \textit{PReLU} apresenta uma tendência maior de se adaptar aos dados. & Apresenta mais parâmetros, aumentando o grau de complexidade da rede neural. \\
            \addlinespace
