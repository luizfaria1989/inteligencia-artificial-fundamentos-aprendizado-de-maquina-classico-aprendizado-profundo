...
\begin{equacaodestaque}{Função de Ativação \textit{Leaky ReLU} (\textit{LReLU})}
...
\begin{equacaodestaque}{Função de Ativação \textit{Parametric ReLU} (\textit{PReLU})}
...
\begin{equacaodestaque}{Derivada da Função de Ativação \textit{Parametric ReLU} (\textit{PReLU})}
...
\begin{equacaodestaque}{Função de Ativação \textit{Exponential Linear Unit} (\textit{ELU})}
...
\begin{equacaodestaque}{Derivada da Função de Ativação \textit{Exponential Linear Unit} (\textit{ELU})}
...
\begin{equacaodestaque}{Função de Ativação \textit{Scaled Exponential Linear Unit} (\textit{SELU})}
...
\begin{itemize}
    \item ReLU da primeira camada: tem como resultado da derivada $\text{ReLU}(z_i) = 1$
    \item ReLU da camada densa 2: tem como resultado da derivada $\text{ReLU}(z_i) = 1$
    \item ReLU da camada densa 3: tem como resultado da derivada $\text{ReLU}(z_i) = 1$
    \item ReLU da camada de saída: tem como resultado da derivada $\text{ReLU}(z_i) = 1$
\end{itemize}

Além disso, você sabe também que o gradiente inicial na camada de saída está sendo de 100. Com isso é possível calcular o gradiente retropropagado para a primeira camada, comecando pela terceira, já que já temos o valor do gradiente para a camada de saída, dessa forma temos que:

\[\begin{WithArrows}
    \delta^{(3)} & = \delta^{(4)} \times \text{ReLU}'(x^{(3)}) \Arrow{Subtituindo os valores} \\
    \delta^{(3)} & = 1 \times 100 = 100
\end{WithArrows}\]
...
\[\begin{WithArrows}
    ...
    \delta^{(1)} & = 100 \times 1 = 100
\end{WithArrows}\]

Perceba que o gradiente não aumentou nem diminuiu, mas ainda sim, um valor de 100, quando comparado aos pequenos valores retornados pelas funções sigmoidais também pode acabar afetando o desempenho e como os pesos e vieses são atualizados em uma rede neural. Esse fenômeno recebe o nome de gradiente explosivo, e pode ser definido como:
