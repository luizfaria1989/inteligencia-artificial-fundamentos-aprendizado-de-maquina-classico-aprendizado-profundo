...
Diferente dos gráficos da BCE que apresentam curvas suaves devido ao uso dos logaritmos, a Figura~\ref{fig:hinge-loss} evidencia que a perda \textit{hinge} é composta por retas. Mais precisamente, essa função de perda tem o formato de uma dobradiça, por isso recebe o nome \textit{hinge}.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Esparsidade:] Semelhante a perda $\epsilon$-insensível (ver Seção-xx) a perda \textit{hinge} promove gradientes esparsos (ver Apêndice-xx). Isso se dá devido à uma zona de perda zero que ocorre com o objetivo de separar os vetores de suporte. Neste caso, os vetores de suporte são os que estão dentro da margem ou foram classificados incorretamente. São eles que irão definir como os parâmetros do modelo serão atualizados.
\end{description}
...
Por utilizar um termo quadrático para avaliar a perda, a Figura~\ref{fig:squared-hinge-loss} mostra que a perda \textit{hinge} quadrática possui um comportamento mais suave; o erro também aumenta de forma mais acelerada. 
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda \textit{hinge} quadrática é uma função de classe $C^1$, \textbf{suave} (ver Apêndice~\ref{ap:deducoes-perda-hinge-quadratica}). Ao elevar a perda \textit{hinge} ao quadrado, é garantido uma função de curvas suaves e sem ``bicos'', como resultado, não é necessário o uso de subgradiente para calcular as derivadas parciais da perda.
    \item[Convexidade:] A perda \textit{hinge} quadrática é uma função \textbf{convexa} (ver Apêndice~\ref{ap:deducoes-perda-hinge-quadratica}). Como essa função de perda é geralmente utilizada em classificação de vetores de suporte, a perda \textit{hinge} quadrática garante apenas um ponto de mínimo
...
Para calcular a sua derivada deve-se aplicar a regra da cadeia, separando então termo ao quadrado da perda \textit{hinge} tradicional. Fazendo isso, encontramos a Equação~\ref{eq:squared-hinge-loss-derivada}.
