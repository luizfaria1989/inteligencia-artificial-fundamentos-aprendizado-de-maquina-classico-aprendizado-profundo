...
Essa função tem como principal objetivo medir a "perda" ou o "excesso" de informação quando é utilizada uma distribuição $q$ para aproximar a distribuição real $q$. Além disso, é a partir dessa fórmula que é possível chegar na definição de Entropia-Cruzada (\textit{Cross-Entropy}), para isso, o primeiro passo, é utilizar as propriedades do logaritmo para reescrever da divergência KL, assim, tem-se:
...
\begin{itemize}
    ...
    \item \textbf{Pune erros confiantes:} \textcite{LossesArticle} explicam que quando $\hat{y}_j$ é perto de 1, mas o valor real é o oposto, neste caso, $y_j = 0$, o termo logarítmico $\log(\hat{y}j)$ ou $\log(1 - \hat{y}_j)$ fica grande em magnitude, penalizando muito os erros confiantes. Essa relação também vale para o contrário, em que $\hat{y}_j$ é perto de 0, mas $y_j = 1$.
...
Também vale a pena discutir o gráfico da derivada da entropia cruzada binária, para isso, ele está representado na Figura \ref{fig:binary-cross-entropy-derivada}.
...
\subsubsection*{Algumas Aplicações da Entropia-Cruzada Binária em Problemas de Classificação Binária}%
...
\subsubsection*{Algumas Aplicações da Entropia-Cruzada Ponderada Binária em Problemas de Classificação Binária}%
\index{Aplicações práticas! Entropia Cruzada Ponderada Binária}
...
Cabe também discutir o gráfico dessa função de perda, o qual é dado pela Figura \ref{fig:hinge-loss}. Perceba que diferente do gráfico da \textit{binary cross-entropy} que fazia uso de logaritmos e por isso apresentava curvas suaves, a \textit{hinge loss} utilizada a função $\max$ e equação da reta, como consequência, o seu gráfico é composto por duas retas que se cruzam no ponto de mínimo.
...
\subsubsection*{Características da \textit{Hinge Loss}}
...
Além da \textit{hinge loss} tradicional, existe uma variante que faz uso de um expoente ao quadrado para calcular a perda. Dessa forma, a perda consegue crescer de forma mais rápida e com isso aumentar a penalização dos erros. Essa é a \textit{squared hinge loss}, a qual está apresentada na seção seguinte.

\subsubsection*{Algumas Aplicações da Hinge Loss em Problemas de Classificação Binária}%
\index{Aplicações práticas! Hinge Loss}
...
\subsubsection*{Algumas Aplicações da Perda Hinge Quadrática em Problemas de Classificação Binária}%
\index{Aplicações práticas! Squared Hinge Loss}
...
\begin{itemize}
    ...
    \item \textbf{Requisitos \textit{one-hot}}: Como dito anteriormente a \textit{CCE} aceita um vetor em formato \textit{one-hot}, isso significa que os valores tanto para $y_j$ quanto para $\hat{y}_j$ devem estar em formato de probabilidade, estando em valores entre zero e um \footnote{Para trabalhar com valores em que os rótulos estão organizados com valores inteiros é possível utilizar uma variante da \textit{categorical cross-entropy}: a \textit{sparse cross-entropy}. Essa função está melhor explicada na Seção \ref{sec:sparse-cross-entropy}}.
\end{itemize}
...
\subsubsection*{Algumas Aplicações da Entropia-Cruzada Categórica em Problemas de Classificação Multi-Classe}%
...
\subsubsection*{Algumas Aplicações da Entropia-Cruzada Categórica Esparsa em Problemas de Classificação Multi-Classe}%
...
\index{Funções de Perda!Wighted Categorical Cross-Entropy (WCCE)}
...
\subsubsection*{Algumas Aplicações da Entropia-Cruzada Categórica Ponderada em Problemas de Classificação Multi-Classe}%
...
\subsubsection*{Características da Perda Multirotulo}
...
\subsubsection*{Algumas Aplicações da Perda Multirótulo em Problemas de Classificação Multi-Label}%
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \midrule
            Entropia Cruzada Binária (\textit{BCE}) & - \\
            \addlinespace
            Entropia Cruzada Ponderada Binária (\textit{WBCE}) & - \\
            \addlinespace
            Perda de Hinge (\textit{Hinge Loss}) & - \\
            \addlinespace
            Entropia Cruzada Categórica (\textit{CCE}) & -  \\
            \addlinespace
