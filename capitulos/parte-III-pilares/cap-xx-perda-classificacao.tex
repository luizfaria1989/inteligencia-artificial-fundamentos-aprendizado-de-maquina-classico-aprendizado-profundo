...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Representação gráfica da derivada da função de perda \textit{hinge} quadrática.}
...
\begin{figure}[h!]
    ...
    \caption{Comportamento da Entropia-cruzada Categórica Ponderada.}%
...
\begin{figure}[h!]
    ...
    \caption{Análise do gradiente da WCCE.}%
...
\section{Perda multi-rótulo (Multi-label loss)}%
\index{Funções de Perda!Perda multi-rótulo (Multi-label loss)}

Imagine que você trabalha em um zoológico e está desenvolvendo uma rede convolucional para identificar os animais em seus viveiros. No total, você tem cerca de 50 diferentes animais e câmeras espalhadas por todo o zoológico. Alguns viveiros são próximos uns dos outros, de forma que em uma mesma filmagem de uma câmera podem ser identificados mais de um animal diferente. Isso é um problema de classificação, mas não se encaixa nem em uma classificação binária nem em uma multi-classe. Mais precisamente, é um problema de classificação multi-rótulo.

Em problemas de classificação multi-rótulo queremos em uma mesma entrada, identificar mais de uma classe presente. Nesses casos, utilizamos a perda multi-rótulo, dada pela Equação~\ref{eq:multilabel-loss}. Essa função de perda faz o somatório de várias BCEs, de forma que para cada classe a ser identificada (no exemplo do zoológico serão aproximadamente 50) existe uma entropia-cruzada binária responsável por avaliar a probabilidade daquela classe estar ou não na entrada avaliada.

\begin{equacaodestaque}{Perda multi-rótulo}
    \mathcal{L}_{\text{multi-label}} = - \sum_{r=1}^{R} [y_r \log(\hat{y}_r) + (1 - y_r) \log(1 - \hat{y}_r)]%
...
Ao utilizar a soma de várias BCEs, a perda multi-rótulo apresenta gráficos parecidos com a entropia-cruzada binária. Na Figura~\ref{fig:multilabel-loss} é mostrado que ela possui o mesmo crescimento logaritmo característico das perdas derivadas da divergência KL.
...
Por ser uma variante da BCE, a perda multi-rótulo herda suas características matemáticas da função primária.

\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda multi-rótulo uma função de classe $C^\infty$, \textbf{infinitamente diferenciável}, no hipercubo aberto $(0, 1)^q$ (ver Apêndice~\ref{ap:deducoes-multilabel-loss}). Como consequência, a perda multi-rótulo pode ser aplicada sem muitos problemas em modelos que são otimizados por gradiente ou por métodos de segunda ordem.
    \item[Convexidade:] A perda multi-rótulo é uma função \textbf{convexa} em relação ao vetor de predições $\mathbf{\hat{y}}$ (ver Apêndice~\ref{ap:deducoes-multilabel-loss}). Essa é uma vantagem para a perda multi-rótulo, pois facilita a convergência dos modelos. Contudo, a convexidade pode ser afetada em modelos de aprendizado profundo devido às transformações não-lineares.
    \item[Robustez:] A perda multi-rótulo não é Lipschitz-contínua, sendo \textbf{sensível a erros confiantes} (ver Apêndice~\ref{ap:deducoes-multilabel-loss}). Portanto, erros confiantes gerados pelo modelo podem enviesar o seu aprendizado e até gerar problemas de gradientes explosivos.
\end{description}

\subsubsection*{Gradiente da perda multi-rótulo}

Semelhante a BCE, na qual utilizamos a sigmoide logística para simplificar os cálculos do gradiente, na perda multi-rótulo podemos aplicar essa mestra estratégia. Fazendo isso, encontramos a Equação~\ref{eq:multilabel-loss-derivada}, neste caso, a somatória está sendo omitida da equação.

\begin{equacaodestaque}{Derivada da Perda Multirrótulo}
    \nabla_{z_i} \mathcal{L}_{\text{multi-label}} (y, \hat{y}) = \hat{y}_i - y_i%
    \label{eq:multilabel-loss-derivada}
\end{equacaodestaque}

Como a Figura~\ref{multilabel-loss-derivada} aponta, a derivada da multi-rótulo para um rótulo é o mesmo gráfico da derivada da BCE. Neste caso, está sendo considerado o gráfico para a Equação~\ref{eq:multilabel-loss-derivada}, na qual é utilizada a sigmoide logística em conjunto com a multi-rótulo. 
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \addlinespace
            Perda multi-rótulo (\textit{Multi-label loss}) & Para problemas de classificação multi-rótulo, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
        \end{tabularx}
