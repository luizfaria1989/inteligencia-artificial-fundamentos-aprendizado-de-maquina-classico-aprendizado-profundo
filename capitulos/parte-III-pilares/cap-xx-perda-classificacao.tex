...
No Capítulo~\ref{cap:perda-regressao} foram vistas diferentes funções de perda para problemas de regressão, os quais dado um conjunto de valores, queremos fazer uma previsão de um novo ponto. Contudo, existem outros tipos de problemas em aprendizado de máquina, um deles consiste em classificar um novo dado em diferentes classes. Eles estão presentes em veículos autônomos, para identificar pedestres e outros objetos na pista. Também estão em diagnósticos de imagens médicas para doenças além de problemas mais simples, como identificar se uma imagem é de um cachorro ou gato.

Este capítulo é dividido em três seções principais. A primeira, explica as perdas para classificação binária, tendo foco da entropia-cruzada binária e a perda \textit{hinge}. A segunda parte, discute as perdas para classificação multi-classe, como a entropia-cruzada categórica. A terceira parte foca em explicar a perda multirrótulo, utilizada para problemas de classificação multirrótulo. Por último, é dedicada uma seção contendo uma tabela resumindo as perdas vistas no capítulo.

\section{Funções de perda para classificação binária}

Um dos problemas de classificação é o de classificação binária. Nele, queremos classificar as informações de um \textit{dataset} em dois grupos. Alguns cenários em que esses problemas incluem são: classificação de imagens de cachorros e gatos, classificação de imagens de exames de pessoas doentes e saudáveis, classificação de áreas desmatadas e áreas florestadas.

Para esses problemas, temos majoritariamente duas escolhas para as funções de perda: as perdas derivadas da Entropia de Shannon (como a BCE e sua variante ponderada), e as perdas de maximização de margem (como a perda \textit{hinge} para as máquinas de vetores de suporte). Desse modo, esta seção foca em explicar essas duas vertentes de funções de perda para classificação binária.

\subsection{Exemplo ilustrativo: cara ou coroa}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Penalização de confiantes:] Quando $\hat{y}$ é perto de 1, mas o valor real é o oposto, neste caso, $y = 0$, o termo logarítmico $\log(\hat{y})$ ou $\log(1 - \hat{y})$ fica grande em magnitude, penalizando muito os erros confiantes. Essa relação também vale para o contrário, em que $\hat{y}$ é perto de 0, mas $y = 1$.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualização do gradiente de perda entropia-cruzada binária no plano.}%
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Gradiente amplificado para erros na classe 0 com $\alpha_0=5.0, \alpha_1=1.0$.}%
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Gradiente amplificado para erros na classe 1 com $\alpha_0=1.0, \alpha_1=5.0$.}%
...
\index{Funções de Perda!Perda \textit{hinge}}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[$f(x)$] representa a saída bruta do modelo ou a função de decisão.
\end{description}
...
Seguindo a analogia da estrada de mão dupla explicada na perda \textit{hinge}, ela se estende também para a sua variante quadrática. Neste caso, essa versão penaliza ainda mais os erros cometidos pelo modelo e também os acertos pouco confiantes (dentro da margem). Diferente da perda \textit{hinge} que calcula o erro linearmente, na \textit{hinge} quadrática o erro é elevado ao quadrado, isso significa que erros confiantes (que cruzam a margem) são fortemente penalizados. Dessa forma, a perda \textit{hinge} quadrática contribui para que o modelo evite ainda mais fazer previsões erradas, especialmente as confiantes.

Essa relação linear x quadrática é a mesma vista no Capítulo~\ref{cap:perda-regressao} ao discutir o erro quadrático médio (MSE) e o erro absoluto médio (MAE). Por esse motivo essas duas perdas também são conhecidas como perda $L_1$ para a \textit{hinge} tradicional e perda $L_2$ para a \textit{hinge} quadrática.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    ...
    \item[Robustez:] A perda \textit{hinge} quadrática não é Lipschitz-contínua, sendo \textbf{sensível a erros confiantes} (ver Apêndice~\ref{ap:deducoes-perda-hinge-quadratica}). Quando a perda \textit{hinge} quadrática é derivada, os termos quadráticos tornam-se retas que tendem ao infinito conforme as probabilidades do modelo aumentam. Assim, erros confiantes podem enviesar o cálculo do gradiente, e até causar problemas de gradientes explosivos. Essa situação é a mesma que acontece com o MAE, que é robusto, mas ao ser elevado ao quadrado tornando-se o MSE, perde a sua robustez.
\end{description}

\subsubsection*{Gradiente da perda hinge quadrática}

O cálculo das derivadas perda \textit{hinge} quadrática são mais simples que a função original. Para encontrar a derivada parcial em relação à saída do modelo devemos aplicar a regra da cadeia, separando o termo quadrático da perda \textit{hinge}. Fazendo isso, encontramos a Equação~\ref{eq:squared-hinge-loss-derivada}.
...
No caso da perda \textit{hinge} quadrática, ela descreve a derivada com apenas duas condições, não sendo necessária uma condição específica para $y \cdot f(x) = 1$. Isso justifica a não utilização do subgradiente na derivação. Com isso, a fronteira entre erro e margem é contínua, o gradiente não muda de forma drástica como acontecia na perda \textit{hinge} tradicional, uma grande vantagem para a \textit{hinge} quadrática e seu uso aliado à otimizadores baseados em gradiente.

A Figura~\ref{fig:squared-hinge-loss-derivada} mostra que o gradiente da perda \textit{hinge} quadrática cresce indefinidamente conforme os valores da saída do modelo aumentam, isso evidencia o fato dessa função de perda não ser Lipschitz-contínua como a perda \textit{hinge} tradicional. Além disso, o seu gráfico é justamente o gráfico da perda \textit{hinge} tradicional porém com inclinações diferentes para a reta afim.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Relação linear por partes do gradiente da função de perda \textit{hinge} quadrática.}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[SVMs com perda \textit{hinge} norma-$p$ (Aprendizado de máquina):] \textcite{sun2024psvmsoftmarginsvmspnorm} discute sobre as $p$SVMs, as quais generalizam a margem suave das SVMs ao introduzir a perda \textit{hinge} norma-$p$, essa nova função de perda tem um parâmetro $p$ cujo objetivo é elevar a perda \textit{hinge} ao exponente $p$. Os autores explicam que ao utilizar a perda \textit{hinge} norma-$p$ com $p = 1$ (perda \textit{hinge}) ou $p = 2$ (perda \textit{hinge} quadrática) o algoritmo de otimização SMO é simplificado em uma expressão linear, garantindo uma performance mais eficiente \parencite{sun2024psvmsoftmarginsvmspnorm}.
    \item[\textit{Gradient boosting} com perda \textit{hinge} quadrática (Aprendizado de máquina):] \textcite{zeng2020fullycorrectivegradientboostingsquared} propõem um algoritmo de \textit{boosting} para classificação binária que utiliza a perda \textit{hinge} quadrática, como justificativa, os autores explicam que essa função permite o princípio da margem.
    \item[Comparativo de funções de perda para classifação em aprendizado profundo (Aprendizado de máquina):] \textcite{janocha2017lossfunctionsdeepneural} investigam como a escolha de funções de perda afetam modelos profundos e suas dinâmicas de aprendizado, em testes realizados pelos autores foi concluído que a perda \textit{hinge} quadrática gerou modelos que poderiam ser treinados mais rapidamente quando comparada com outras funções, como a entropia-cruzada e a perda $L_1$. Além disso, as perdas que trabalham com a maximização da margem possuem a tendência de generalizar melhor, ultrapassando as outras famílias analisadas \parencite{janocha2017lossfunctionsdeepneural}.
\end{description}

\section{Funções de perda para classificação multi-classe}

As perdas vistas até o momento são utilizadas em cenários nos quais temos apenas duas classes para serem identificadas. Mas, vão existir situações onde teremos um conjunto maior de classes para serem analisadas. Nesses casos, utilizaremos perdas de classificação multi-classe, como a entropia-cruzada categórica e as suas variações. 

\subsection{Exemplo ilustrativo: corrida de cavalos}

Imagine que você decidiu apostar em uma corrida de cavalos. São cinco cavalos, e para cada um você deve dar uma porcentagem da probabilidade dele ganhar, além disso, a soma das porcentagens deve ser sempre um.  O valor em dinheiro que você irá receber será correspondente ao valor da porcentagem escolhida para o cavalo ganhador.

Nesse jogo, a entropia-cruzada categórica é a juíza. Ela compara as suas previsões com um gabarito composto por zeros e uns (em formato \textit{hot-one encoding}) e gera a perda, indicando o quanto os parâmetros devem ser ajustados para a próxima iteração. 

Ainda existe um segundo participante nessa tarefa de classificação: a função \textit{softmax}. A \textit{softmax} transforma as saídas do modelo no seu gabarito, com probabilidades para cada uma das classes que quando somadas serão sempre um.

Juntas, a CCE e a \textit{softmax} são umas das principais funções para serem utilizadas em problemas de classificação multi-classe.

\subsection{Entropia-cruzada categórica (CCE)}%
\index{Funções de Perda!Entropia-cruzada categórica (CCE)}

A \textbf{entropia-cruzada categórica}, também chamada de \textbf{\textit{categorical cross-entropy}} (\textbf{CCE}), estende os conceitos da BCE para lidar com $C$ diferentes classes em uma representação em formato \textit{one-hot}. A formatação \textit{one-hot} funciona da seguinte forma: para cada uma das classes presentes, é adicionada uma coluna binária responsável por marcar 1 se há presença daquela classe, ou 0, se há ausência da classe. Um exemplo dessa codificação está na Tabela~\ref{tab:one-hot-encoding-exemplo}.
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[$y_c$] representa o rótulo para a classe $c$, segue o formato $y \in \{0, +1\}$;
    \item[$\hat{y}_c$] representa a probabilidade prevista para a classe $c$;
    \item[$C$] representa o número de classes.
\end{description}

Em um problema em que só existe uma classe correta para ser prevista, todos os valores de $y_c$ exceto para a amostra correta serão zero. Dessa forma, a CCE é simplificada para o cálculo

\[
    y \log(\hat{y})
\]

ou seja, ela irá calcular a distância que o modelo chegou da classe correta. Assim, a CCE ajusta individualmente os pesos para ao ser definida qual classe é a correta em uma predição.

Representar o gráfico da CCE em uma figura é algo complexo, visto que, ele pode variar conforme o número de classes existentes. Considerando isso, a Figura~\ref{fig:categorical-cross-entropy} apresenta o gráfico contendo apenas uma entropia-cruzada, com a sua curva logarítmica. O gráfico é o mesmo da entropia-cruzada binária, visto que a CCE é uma extensão dessa outra função de perda.
...
\begin{figure}
    ...
    \caption{Visualização da função de perda entropia-cruzada categórica no plano para uma única entropia-cruzada.}%
...
Para a derivada, aplicamos a regra da cadeia, pois, a CCE depende das previsões $\hat{y}_j$ e as previsões, por sua vez, dependem dos \textit{logits}. Escrevemos a regra da cadeia como sendo
...
A partir dessa regra da cadeia, teremos dois termos para serem calculados, o gradiente da perda em relação à predição e o jacobiano da \textit{softmax}. Assim, dividimos em dois passos a dedução.

\textbf{Passo 1: o gradiente da perda em relação à predição}
...
\textbf{Passo 2: o jacobiano da softmax}

A derivada da \textit{softmax} se comporta de duas maneiras diferentes, as quais dependem se estamos olhando para a mesma classe ($i = j$) ou classes diferentes ($i \neq j$):

\begin{enumerate}
    \item \textbf{Mesma classe ($i = j$):}  A derivada é $\hat{y}_i (1 - \hat{y}_i)$;
    \item \textbf{Classes diferentes ($i \neq j$):} A derivada é $\hat{y}_j \hat{y}_i$
\end{enumerate}

\textbf{Simplificando}

Substituindo as equações encontradas no passo 1 e no passo 2 na regra da cadeia encontramos:

\[
    \frac{\partial \mathcal{L}}{\partial z_i} = 
\underbrace{ \left( -\frac{y_i}{\hat{y}_i} \right) \hat{y}_i(1 - \hat{y}_i) }_{\text{Caso } j=i} 
+ \sum_{j \neq i} 
\underbrace{ \left( -\frac{y_j}{\hat{y}_j} \right) (-\hat{y}_j\hat{y}_i) }_{\text{Caso } j \neq i}
\]

Podemos simplificar os termos com $\hat{y}$ no denominador com os termos com $\hat{y}$ no numerador, assim escrevemos:
...
Colocamos $\hat{y}_i$ em evidência nos termos positivos:

\[
    \frac{\partial \mathcal{L}}{\partial z_i} = -y_i + \hat{y}_i 
    \underbrace{\left( y_i + \sum_{j \neq i} y_j \right)}_{\text{Soma de todas as probabilidades reais}}
\]

Como $y$ é um vetor \textit{one-hot}, a soma de todos os $y$ deve ser igual a 1. Logo, o termo em parênteses é simplificado para 1 chegando finalmente na Equação~\ref{eq:category-cross-entropy-derivada}. 
...
A Figura~\ref{fig:categorical-cross-entropy-derivada-com-softmax} aponta que essa simplificação do gradiente da CCE com a \textit{softmax} reflete também no gráfico da função, sendo composto por uma reta afim ou no caso tridimensional, de um plano. Ao utilizar a \textit{softmax} em conjunto com a CCE, simplificamos as os cálculos do gradiente além de garantir que a saída do modelo esteja no intervalo correto ($y \in {0, +1}$) para ser calculada a perda.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Relação linear do gradiente da função de perda entropia-cruzada categórica ao utilizar a função \textit{softmax} na camada de saída.}%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Classificação de imagens de lesões hepáticas (Saúde):] \textcite{heker2020jointliverlesionsegmentation} propõem um \textit{joint framework} para classificar em três tipos de lesões hepáticas, superando algoritmos como Y-Net, por se tratar de um problema de classificação multi-classe, os autores utilizam a CCE como função de perda.
    \item[Classificação de imagens de células sanguíneas (Saúde):] \textcite{becirovic2025performancecomparisonmedicalimage} comparam a performance dos \textit{frameworks} TensorFlow com Keras, PyTorch e JAX, na classificação em oito classes de imagens de células sanguíneas no \textit{BloodMINNIST dataset}, os autores optaram pela CCE para calcular a perda do modelo.
    \item[Classificação de doenças em folhas de tomateiro (Agronomia):] \textcite{oni2025optimizedcustomcnnrealtime} criam uma rede neural convolucional para classificar diferentes tipos de doenças que podem ocorrer nas folhas de tomateiros, os autores utilizam a CCE como função de perda para avaliar os erros do modelo.
\end{description}
...
A \textbf{entropia cruzada categórica esparsa} é utilizada para os cenários em que os rótulos das classes são dados em inteiros $y_c \in {1, 2, \cdots C}$ \parencite{LossesArticle}. Diferente da CCE padrão que utiliza zeros e uns para zerar as parcelas do somatório e sobrar apenas a análise da perda para a classe verdadeira, a CCE esparsa faz isso de forma automática. Ela usa os rótulos inteiros para encontrar qual é a classe verdadeira, simplificando a notação de somatório. 

Dessa forma, a CCE esparsa é definida pela Equação~\ref{eq:sparse-categorical-cross}.

\begin{equacaodestaque}{Entropia-cruzada categórica esparsa}
    \mathcal{L}_{\text{sparse cce}}(y, \hat{y}) = - \log(\hat{y}_c)%
    \label{eq:sparse-categorical-cross}
\end{equacaodestaque}
...
Mesmo utilizando fórmulas diferentes, ambas as entropias-cruzadas categóricas calculam a perda de forma semelhante, assim como a BCE e as outras funções derivadas da divergência KL. Por isso, os gráficos da CCE esparsa bem como a sua derivada serão omitidos dessa seção.

\subsubsection*{Características da entropia-cruzada categórica esparsa}

\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Similaridades com a CCE:] A CCE esparsa herda as características matemáticas da sua variante que recebe entradas em formato \textit{one-hot}. Essa função é de classe $C^{\infty}$, \textbf{infinitamente diferenciável}, é \textbf{convexa} em relação predições $\hat{y}$, e não é Lipschitz-contínua, sendo \textbf{sensível a erros confiantes} (ver Apêndice~\ref{ap:deducoes-perdas-derivadas-da-kl-divergence}).
    \item[Eficiência:] \textcite{LossesArticle} explicam que em problemas de classificação com um grande número de classes, a codificação \textit{one-hot} pode ser exigir muita memória, ao utilizar a \textit{sparse CCE}, os seus indexes que já apontam diretamente para a probabilidade conseguem escapar de trabalhar com dados em formato \textit{one-hot}. Portanto, uma justificativa para optar pela CCE esparsa é o desempenho computacional que essa função pode trazer, algo que pode ser notório em grandes modelos e em modelos que serão utilizados em sistemas cuja memória é muito limitada.
\end{description}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Predição de hashtags em e-comerces (Comércio):] \textcite{Camargo_2021} estudam sobre modelos como BiLSTM e BERT podem predizer hashtags em português brasileiro para sites de \textit{e-comerce}, os autores aplicam uma tarefa de classificação utilizando a CCE esparsa com o intuito de refinar o BERT e sua habilidade de predizer palavras.
    \item[Classificação de grãos de arroz (Agronomia):] \textcite{xia2025improvedpurefullyconnected} desenvolvem uma rede neural totalmente conectada para classificação de grãos de arroz, como se tratava de um problema de classificação multi-classe, os autores optam pela CCE esparsa como função de perda.
    \item[Criação de CNN em sistemas IoT para detecção de intrusos (Segurança):] \textcite{naeem2025efficientiotintrusiondetection} criam uma CNN com arquitetura BiLSTM e recursos de atenção para identificar tipos de intrusão em sistemas IoT, os autores utilizam a CCE esparsa como função de perda. Em sistemas que possuem uma quantidade de memória RAM limitada, como em IoT, a CCE esparsa consegue ajudar a economia de memória. 
\end{description}

\subsection{Entropia-cruzada categórica ponderada (WCCE)}%
\index{Funções de Perda!Entropia-cruzada categórica ponderada (WCCE)}

No exemplo dessa seção a CCE foi explicada com jogo de apostas em uma corrida de cavalos. Naquele caso, foi considerado que todos os cavalos tinham a mesma chance de ganhar, eram igualmente bons. Mas em cenários em que alguns cavalos são melhores ou piores que a média, a CCE não seria a função ideal para julgar as apostas da corrida. Um cavalo ruim, deveria gerar maiores lucros para quem apostasse nele caso ganhasse, seria puramente sorte, e uma aposta arrisca. Por outro lado, ao apostar em um cavalo bom, existe uma grande chance de ganhar a aposta, não tem um alto risco. 

Nessas situações, nas quais uma classe é melhor que outra, ou, no caso de aprendizado de máquina, que uma é mais frequente que a outra, devemos aplicar a ponderação de pesos para a CCE. Nesse sentido, é apresentada a \textbf{entropia-cruzada categórica ponderada} ou \textit{\textbf{weighted categorical cross-entropy}} (\textbf{WCCE}), que funciona de forma semelhante à WBCE, a diferente, é que ela lida com classificação multi-classe, ao invés de binária.

A fórmula da WCCE é dada pela Equação~\ref{eq:weighted-categorical-cross-entropy}. Contudo, como será visto nos exemplos de usos dessa função, diferentes técnicas são utilizadas para a ponderação dos pesos, criando fórmulas diferentes. Na prática, essas funções seguem o mesmo raciocínio: utilizar pesos nas classes para lidar com o desbalanceamento.

\begin{equacaodestaque}{Entropia-cruzada categórica ponderada}
    \mathcal{L}_{\text{wcce}}(y, \hat{y}) = - \sum_{c=1}^{C} \alpha_c y_c \log(\hat{y}_c)%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[$y_c$] representa o rótulo para a classe $c$, segue o formato $y \in \{0, +1\}$;
    \item[$\hat{y}_j$] representa a probabilidade prevista para a classe $c$;
    \item[$\alpha_c$] representa o peso atribuído para a classe $c$; 
    \item[$C$] representa o número de classes.
\end{description}

Os pesos podem afetar consideravelmente como as curvas da WCCE crescem ou diminuem. Como a Figura~\ref{comparativo-wcce} apresenta, um peso alto gera uma curva mais inclinada e que cresce mais rapidamente para o infinito. Certamente esse curva será de uma classe que apresenta menor frequência no \textit{dataset}, que busca compensar esse fato com um peso maior, gerando maiores atualizações do gradiente. Por outro lado, quanto menor é o peso, mais próxima a curva fica do eixo das abscissas, com comportamento que se assemelha ao de uma reta constante.

\begin{figure}[h!]
    \centering
    % Figura da Esquerda: Enfatizando Classes Raras (Alpha > 1)
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \def\alphaNormal{1.0} % Peso padrão
            \def\alphaAlta{5.0}   % Peso alto (ex: classe minoritária)
            \begin{axis}[
                xlabel={Probabilidade da Classe Correta ($\hat{y}_c$)},
                ylabel={Perda $\mathcal{L}_{wcce}$},
                axis lines=left,
                grid=major,
                grid style={dashed, gray!40},
                xmin=0, xmax=1,
                ymin=0, ymax=15, % Focado na região de erro
                legend pos=north east,
                width=\textwidth,
                label style={font=\small},
                tick label style={font=\scriptsize},
                ...
                % Curva para Peso Alto (Classe Rara)
                \addplot[
                    domain=0.01:1.0, samples=100, color=black, very thick
                ] {-\alphaAlta*ln(x)};
                \addlegendentry{$\alpha_c=5.0$}

                % Curva para Peso Normal
                \addplot[
                    domain=0.01:1.0, samples=100, color=black, dashed
                ] {-\alphaNormal*ln(x)};
                \addlegendentry{$\alpha_c=1.0$}
            \end{axis}
        \end{tikzpicture}
        \caption{Priorização de classes raras: o erro é amplificado ($\alpha > 1$).}%
        \label{fig:wcce-alto-peso}
    \end{subfigure}
    \hfill 
    % Figura da Direita: Atenuando Classes Frequentes (Alpha < 1)
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \def\alphaNormal{1.0} % Peso padrão
            \def\alphaBaixa{0.2}  % Peso baixo (ex: fundo/background)
            \begin{axis}[
                xlabel={Probabilidade da Classe Correta ($\hat{y}_c$)},
                ylabel={Perda $\mathcal{L}_{wcce}$},
                axis lines=left,
                ...
                grid style={dashed, gray!40},
                xmin=0, xmax=1,
                ymin=0, ymax=15, % Mesma escala para comparação
                legend pos=north east,
                width=\textwidth,
                label style={font=\small},
                tick label style={font=\scriptsize},
                ...
                % Curva para Peso Baixo (Classe Frequente)
                \addplot[
                    domain=0.01:1.0, samples=100, color=black, very thick
                ] {-\alphaBaixa*ln(x)};
                \addlegendentry{$\alpha_c=0.2$}
            \end{axis}
        \end{tikzpicture}
        \caption{Atenuação de classes frequentes: o erro é reduzido ($\alpha < 1$).}%
        \label{fig:wcce-baixo-peso}
    \end{subfigure}
    
    \caption{Comparação das curvas da entropia-cruzada categórica ponderada para diferentes valores de pesos.}%
    \label{fig:comparativo-wcce}
\end{figure}

\subsubsection*{Características da entropia-cruzada categórica ponderada}

A WCCE herda as características matemáticas da CCE, que por sua vez herda da BCE.

\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Similaridades com a CCE:] Por ser uma variante da CCE, a WCCE possui as mesmas características matemáticas da função original. Assim, a A WCCE é de classe $C^{\infty}$, \textbf{infinitamente diferenciável} no domínio aberto $\hat{y}_i \in (0, 1)$, é \textbf{convexa} em relação predições $\hat{y}$ e não é Lipschitz-contínua, sendo \textbf{sensível a erros confiantes} (ver Apêndice~\ref{ap:deducoes-perdas-derivadas-da-kl-divergence}).
    \item[Atenção os pesos:] Sendo uma perda ponderada, a CCE apresenta curvas que crescem de forma diferente. Desse modo, a classe que recebe os maiores pesos, pode ser responsável por causar enviesamento do modelo, caso os dados estejam rotulados incorretamente. Isso acontece, porque essa classe será mais sensível aos erros, apresentando uma curva com crescimento mais acelerado e também um maior gradiente. 
\end{description}

\subsubsection*{Gradiente da entropia-cruzada categórica ponderada}

Ao calcular a derivada parcial da WCCE em relação à predição, encontramos a Equação~\ref{eq:grad-weighted-categorical-cross-entropy}. No gradiente acontece o comportamento inverso, o parâmetro $\alpha_c$ é responsável por definir se a curva do gradiente será mais ou menos próxima do eixo das ordenadas.

\begin{equacaodestaque}{Gradiente da entropia-cruzada categórica ponderada em relação à predição}
    \nabla_{\hat{y}_c} \mathcal{L}_{\text{wcce}}(y, \hat{y}) = - \alpha_c \frac{y_c}{\hat{y}_c}
    \label{eq:grad-weighted-categorical-cross-entropy}
\end{equacaodestaque}

Assim como as outras perdas categóricas, a WCCE também pode ser utilizada em conjunto com a \textit{softmax}, encontrando como derivada a Equação~\ref{eq:grad-weighted-categorical-cross-entropy}\footnote{A derivação exata com a \textit{softmax} e pesos arbitrários é mais complexa pois o peso de uma classe afeta o gradiente das outras devido à normalização da \textit{softmax}. Na prática, o erro, dado pelo termo ($\hat{y} - y$)} será escalado pelo peso $\alpha$.

\begin{equacaodestaque}{Gradiente da entropia-cruzada categórica ponderada em relação à predição com softmax}
    \nabla_{z_i} \mathcal{L}_{\text{wcce}}(y, \hat{y}) = - \alpha_c (\hat{y}_i - y_i)
    \label{eq:grad-weighted-categorical-cross-entropy-with-softmax}
\end{equacaodestaque}

Os gráficos da derivada da WCCE indicam o tamanho da correção a ser feita conforme o modelo vai errando. A Figura~\ref{fig:wcce-derivada-comparacao} mostra que para classes abundantes, as correções são mais suaves, com um gradiente menor. Entretanto, para as classes raras, o gradiente é maior, e também será a correção.

\begin{figure}[h!]
    \centering
    % Figura da Esquerda: Gradiente Agressivo (Peso Alto)
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \def\alphaNormal{1.0} 
            \def\alphaAlta{5.0}   
            \begin{axis}[
                title={Gradiente para Classes Raras ($\alpha > 1$)},
                xlabel={Probabilidade Prevista ($\hat{y}_c$)},
                ylabel={Gradiente ($\nabla_{\hat{y}} \mathcal{L}$)},
                axis lines=left,
                grid=major,
                grid style={dashed, gray!40},
                xmin=0, xmax=1.1,
                ymin=-60, ymax=0, % Gradiente é negativo (quer aumentar p)
                ...
                % Derivada Peso Alto
                \addplot[
                    domain=0.08:1, samples=100, color=black, very thick
                ] {-\alphaAlta/x};
                \addlegendentry{$\alpha_c=5.0$}

                % Derivada Peso Normal
                \addplot[
                    domain=0.02:1, samples=100, color=black, dashed
                ] {-\alphaNormal/x};
                \addlegendentry{$\alpha_c=1.0$}
            \end{axis}
        \end{tikzpicture}
        \caption{Gradiente amplificado: O modelo recebe um sinal de correção 5x maior.}%
        \label{fig:wcce-derivada-alto-peso}
    \end{subfigure}
    \hfill
    % Figura da Direita: Gradiente Suave (Peso Baixo)
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \def\alphaNormal{1.0} 
            \def\alphaBaixa{0.2}   
            \begin{axis}[
                title={Gradiente para Classes Frequentes ($\alpha < 1$)},
                xlabel={Probabilidade Prevista ($\hat{y}_c$)},
                ylabel={Gradiente ($\nabla_{\hat{y}} \mathcal{L}$)},
                axis lines=left,
                grid=major,
                ...
                % Derivada Peso Baixo
                \addplot[
                    domain=0.01:1, samples=100, color=black, very thick
                ] {-\alphaBaixa/x};
                \addlegendentry{$\alpha_c=0.2$}
            \end{axis}
        \end{tikzpicture}
        \caption{Gradiente atenuado: O erro gera um sinal de correção pequeno.}%
        \label{fig:wcce-derivada-baixo-peso}
    \end{subfigure}
    
    \caption{Comparação das curvas do gradiente da entropia-cruzada categórica ponderada para diferentes valores dos pesos.}%
    \label{fig:wcce-derivada-comparacao}
\end{figure}
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Segmentação de imagens médicas (Saúde):] \textcite{hosseini2024dilatedbalancedcrossentropy} utilizam um tipo de CCE ponderada chamada Dilated Balanced Cross Entropy para segmentação de imagem médica. a ponderação da DBCE para cada classe é baseada em uma área dilatada da máscara de cada classe, e pesos de balanceamento são atribuídos a cada classe juntamente com seus pixels vizinhos.
    \item[Comparativo de perdas ponderadas em detecção de objetos (Veículos autônomos):] \textcite{phan2020resolvingclassimbalanceobject} exploram o uso de diferentes perdas CCE ponderadas, como Balanced Cross Entropy, Focal Loss e Class-Balanced Loss, em problemas de detecção de objetos no \textit{dataset} BDD100K, contendo imagens de câmeras de carros e outros veículos. 
    \item[Classificação de expressões faciais (Área):] \textcite{farabi2025insideoutefficientnetv2sbaseddeep} desenvolvem uma EfficientNetV2-S para a classificação em sete classes de expressões faciais, para lidar com os dados desbalanceados no \textit{dataset}, os autores utilizam a class-weighted categorical cross entropy como perda.
\end{description}

\section{Perda multirrótulo (Multilabel loss)}%
\index{Funções de Perda!Perda multirrótulo (Multilabel loss)}

Imagine que você trabalha em um zoológico e está desenvolvendo uma rede convolucional para identificar os animais em seus viveiros. No total, você tem cerca de 50 diferentes animais e câmeras espalhadas por todo o zoológico. Alguns viveiros são próximos uns dos outros, de forma que em uma mesma filmagem de uma câmera podem ser identificados mais de um animal diferente. Isso é um problema de classificação, mas não se encaixa nem em uma classificação binária nem em uma multi-classe. Mais precisamente, é um problema de classificação multirrótulo.

Em problemas de classificação multirrótulo queremos em uma mesma entrada, identificar mais de uma classe presente. Nesses casos, utilizamos a perda multirrótulo, dada pela Equação~\ref{eq:multilabel-loss}. Essa função de perda faz o somatório de várias BCEs, de forma que para cada classe a ser identificada (no exemplo do zoológico serão aproximadamente 50) existe uma entropia-cruzada binária responsável por avaliar a probabilidade daquela classe estar ou não na entrada avaliada.

\begin{equacaodestaque}{Perda multirrótulo}
    \mathcal{L}_{\text{multilabel}} = - \sum_{r=1}^{R} [y_r \log(\hat{y}_r) + (1 - y_r) \log(1 - \hat{y}_r)]%
...
\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[$y_r$] representa o rótulo real para o rótulo $r$, onde $y_r \in \{0, 1\}$;
    \item[$\hat{y}_r$] representa a probabilidade prevista para o rótulo $r$;
    \item[$R$] representa o número total de rótulos possíveis.
\end{description}

Ao utilizar a soma de várias BCEs, a perda multirrótulo apresenta gráficos parecidos com a entropia-cruzada binária. Na Figura~\ref{fig:multilabel-loss} é mostrado que ela possui o mesmo crescimento logaritmo característico das perdas derivadas da divergência KL.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualização da perda multirrótulo no plano.}%
    \label{fig:multilabel-loss}
    \fonte{O autor (2026).}
\end{figure}

\subsubsection*{Características da perda multirrótulo}

Por ser uma variante da BCE, a perda multirrótulo herda suas características matemáticas da função primária.

\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Continuidade, suavidade e diferenciabilidade:] A perda multirrótulo uma função de classe $C^\infty$, \textbf{infinitamente diferenciável}, no hipercubo aberto $(0, 1)^q$ (ver Apêndice~\ref{ap:deducoes-multilabel-loss}). Como consequência, a perda multirrótulo pode ser aplicada sem muitos problemas em modelos que são otimizados por gradiente ou por métodos de segunda ordem.
    \item[Convexidade:] A perda multirrótulo é uma função \textbf{convexa} em relação ao vetor de predições $\mathbf{\hat{y}}$ (ver Apêndice~\ref{ap:deducoes-multilabel-loss}). Essa é uma vantagem para a perda multirrótulo, pois facilita a convergência dos modelos. Contudo, a convexidade pode ser afetada em modelos de aprendizado profundo devido às transformações não-lineares.
    \item[Robustez:] A perda multirrótulo não é Lipschitz-contínua, sendo \textbf{sensível a erros confiantes} (ver Apêndice~\ref{ap:deducoes-multilabel-loss}). Portanto, erros confiantes gerados pelo modelo podem enviesar o seu aprendizado e até gerar problemas de gradientes explosivos.
\end{description}

\subsubsection*{Gradiente da perda multirrótulo}

Semelhante a BCE, na qual utilizamos a sigmoide logística para simplificar os cálculos do gradiente, na perda multirrótulo podemos aplicar essa mestra estratégia. Fazendo isso, encontramos a Equação~\ref{eq:multilabel-loss-derivada}, neste caso, a somatória está sendo omitida da equação.

\begin{equacaodestaque}{Derivada da Perda Multirrótulo}
    \nabla_{z_i} \mathcal{L}_{\text{multilabel}} (y, \hat{y}) = \hat{y}_i - y_i%
    \label{eq:multilabel-loss-derivada}
\end{equacaodestaque}

Como a Figura~\ref{fig:multilabel-loss-derivada} aponta, a derivada da multirrótulo para um rótulo é o mesmo gráfico da derivada da BCE. Neste caso, está sendo considerado o gráfico para a Equação~\ref{eq:multilabel-loss-derivada}, na qual é utilizada a sigmoide logística em conjunto com a multirrótulo. 
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Relação linear por partes do gradiente da perda multirrótulo para um rótulo.}%
    \label{fig:multilabel-loss-derivada}
    \fonte{O autor (2026).}
\end{figure}

\subsubsection*{Aplicações da perda multirrótulo}%
\index{Aplicações práticas!Perda multirrótulo (Multilabel loss)}

\begin{description}[style=sameline, leftmargin=1.5em, font=\bfseries\color{black}] 
    \item[Reconhecimento de múltiplos objetos em imagens (Visão computacional):] \textcite{zhu2023multilabelselfsupervisedlearningscene} propõem o método \textit{Multi-Label Self-supervised learning} (MLS) para o reconhecimento de múltiplos objetos em uma única imagem, os autores aplicam diversas BCEs para permitir a identificação dos diferentes objetos do \textit{dataset} MS-COCO, desse modo, o MLS atinge resultados estado-da-arte nos \textit{benchmarks}.
    \item[Reconhecimento de múltiplas patologias em raios-x toráxicos (Saúde):] \textcite{asadi2025clinicallyinspiredhierarchicalmultilabelclassification} desenvolvem \textit{Hierarchical Binary Cross-Entropy} (HBCE), uma função de perda que une a entropia-cruzada binária com termos de penalidade, como justificativa, os autores explicam que em tarefas de classificação multirrótulo como em raios-x torácicos, a presença de uma doença pode aumentar a presença de outra (como no caso de efusão pleural e pneumonia) criando uma espécie de hierarquia entre as patologias.
    \item[Reconhecimento de múltiplos sons em um clipe de aúdio (Engenharia de aúdio):] \textcite{hou2018audiotagging} trabalham com a BCE para a etiquetagem de áudio com objetivo de identificar efeitos sonoros em um clipe, para isso eles criam uma rede neural convolucional recorrente (CRNN) e utilizam a BCE em cada \textit{frame} de modo que a perda total é dada pela soma das BCEs \textit{frame} a \textit{frame} permitindo a identificação de diferentes sons em um clipe.
\end{description}
...
\begin{table}[htbp]
    \centering
    \begin{threeparttable}
        \caption{Comparativo das funções de perda para classificação}%
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \midrule
            Entropia-cruzada binária (BCE) & Para classificação binária com classes de mesma frequência, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Entropia-cruzada binária ponderada (\textit{WBCE}) & Para classificação binária com classes de frequências diferentes, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Perda \textit{hinge} (\textit{Hinge loss}) & Para classificação binária com SVMs, contínua ($C^0$), robusta a erros confiantes, convexa, promove a esparsidade do gradiente \\
            \addlinespace
            Perda \textit{hinge} quadrática (\textit{Squared hinge loss}) & Para classificação binária com SVMs, suave ($C^1$), sensível a erros confiantes, convexa \\
            \addlinespace
            Entropia-cruzada categórica (CCE) & Para classificação multi-classe com rótulos em formato \textit{one-hot encoding}, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa.  \\
            \addlinespace
            Entropia-cruzada categórica esparsa (\textit{Sparse CCE}) & Para classificação multi-classe com rótulos inteiros, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Entropia-cruzada categórica ponderada (\textit{WCCE}) & Para classificação multi-classe com classes de frequências diferentes, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Perda multirrótulo (\textit{Multi-label loss}) & Para classificação multirrótulo, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
        \end{tabularx}
