...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{p{3.2cm} *{1}{>{\raggedright\arraybackslash}X}}
            ...
            \midrule
            Entropia-cruzada binária (BCE) & Para problemas de classificação binária com classes de mesma frequência, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Entropia-cruzada binária ponderada (\textit{WBCE}) & Para problemas de classificação binária com classes de frequências diferentes, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Perda \textit{hinge} (\textit{Hinge loss}) & Para problemas de classificação binária com SVMs, contínua ($C^0$), robusta a erros confiantes, convexa, promove a esparsidade do gradiente \\
            \addlinespace
            Perda \textit{hinge} quadrática (\textit{Squared hinge loss}) & Para problemas de classificação binária com SVMs, suave ($C^1$), sensível a erros confiantes, convexa \\
            \addlinespace
            Entropia-cruzada categórica (CCE) & Para problemas de classificação multi-classe com rótulos em formato \textit{one-hot encoding}, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa.  \\
            \addlinespace
            Entropia-cruzada categórica esparsa (\textit{Sparse CCE}) & Para problemas de classificação multi-classe com rótulos inteiros, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Entropia-cruzada categórica ponderada (\textit{WCCE}) & Para problemas de classificação multi-classe com classes de frequências diferentes, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
            Perda multirrótulo (\textit{Multi-label loss}) & Para problemas de classificação multirrótulo, infinitamente diferenciável ($C^{\infty}$), sensível a erros confiantes, convexa. \\
            \addlinespace
        \end{tabularx}
