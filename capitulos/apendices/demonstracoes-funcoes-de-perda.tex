...
Observamos que Binary Cross-Entropy (BCE), Categorical Cross-Entropy (CCE) e suas variantes ponderadas (WBCE, WCCE, ECCE) compartilham a mesma estrutura algébrica fundamental. Elas são instâncias da \textit{Entropia Cruzada Generalizada}.
...
\begin{proposition}[Regularidade Global]
    A família de perdas de Entropia Cruzada pertence à classe $C^\infty$ (suave) no domínio aberto $\hat{y}_i \in (0, 1)$.
\end{proposition}
...
\begin{proposition}[Não-Robustez e Explosão de Gradiente]
    A família de perdas de Entropia Cruzada não é Lipschitz-contínua, sendo sensível a erros confiantes (\textit{confident mistakes}).
\end{proposition}
...
\begin{definition}
    Em classificação binária onde $y \in \{-1, +1\}$ e $f(x) \in \mathbb{R}$ é o score bruto do modelo. Definimos a margem funcional como $z = y \cdot f(x)$. A Hinge Loss é definida por:
    \begin{equation}
...
\begin{proposition}[Singularidade]
    A função Hinge Loss pertence à classe $C^0$ (contínua), mas não pertence à classe $C^1$.
\end{proposition}
...
\begin{proposition}[Robustez e Esparsidade Dual]
    A função Hinge Loss é Lipschitz-contínua e induz esparsidade nos vetores de suporte.
\end{proposition}
...
\begin{definition}
    Mantendo a definição de margem funcional $z = y \cdot f(x)$, a função de perda Squared Hinge é definida pelo quadrado da parte positiva da Hinge padrão:
    \begin{equation}
...
\begin{proposition}[Suavização e Regularidade]
    A função Squared Hinge pertence à classe $C^1$ (continuamente diferenciável), superando a singularidade da Hinge padrão. Porém, não é $C^2$.
\end{proposition}
...
\begin{proposition}[Convexidade]
    A função Squared Hinge é convexa.
\end{proposition}
...
\begin{proposition}[Perda de Robustez]
    Diferente da Hinge padrão, a Squared Hinge \textbf{não é Lipschitz-contínua}, sendo sensível a \textit{outliers}.
\end{proposition}
...
\subsection{Perda multirótulo (multilabel loss)}%
...
\begin{proposition}[Regularidade e Independência]
    A função Multilabel Loss pertence à classe $C^\infty$ no hipercubo aberto $(0, 1)^q$.
\end{proposition}
...
\begin{proposition}[Convexidade Aditiva]
    A função Multilabel Loss é convexa em relação ao vetor de predições $\mathbf{\hat{y}}$.
\end{proposition}
...
\begin{proof}
    ...
    2.  A função Multilabel é a soma: $\mathcal{L} = \sum_{j=1}^q \ell_j(\hat{y}_j)$.
...
\begin{proposition}[Não-Robustez]
    A função Multilabel Loss não é Lipschitz-contínua e herda a sensibilidade a \textit{outliers} da BCE.
\end{proposition}
